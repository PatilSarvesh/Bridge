...
The API, standalone MCP service, and worker expose dependency-free Prometheus text metrics at `GET /metrics`; the worker defaults to `127.0.0.1:4200` and validates bounded `BRIDGE_WORKER_METRICS_HOST`/`BRIDGE_WORKER_METRICS_PORT` overrides. Instrumentation covers request latency/errors/authorization denials, bounded MCP initialize and tool-call outcomes/durations, context performance, repository transactions, outbox processing, and notification delivery without request arguments, tenant, project, principal, record, or content labels. Importable pilot dashboard and alert definitions live in `config/observability`; initial non-contractual objectives are in [`docs/service-objectives.md`](docs/service-objectives.md). Restrict every deployed scrape endpoint to the monitoring network. See [`docs/observability.md`](docs/observability.md) for exact behavior and remaining deployment work.
