...
- `doctor` diagnostics for API and repository readiness, project mapping, versioned generated
  instructions, adapter markers, and actionable loopback-sandbox recovery.
...
The implemented CLI flow uses a separate native/public client ID and never receives the confidential web client secret. The API publishes only public CLI configuration. The CLI binds an exact `http://127.0.0.1:<port>/<path>` redirect, validates state, exchanges the code with S256 PKCE, asks Bridge to validate the resulting bearer token and active membership, then stores a bounded versioned session in macOS Keychain or Linux Secret Service. Near-expiry access tokens refresh when an offline refresh token is available; rejected or non-refreshable sessions are removed and require login. Logout attempts provider refresh-token revocation before clearing local storage. Web callback success and cookie-backed logout append organization-scoped metadata audit events only after a trusted human principal is resolved. Failed or unknown requests emit only bounded authentication outcomes and safe correlation-aware logs without tenant attribution or persisted credential material; they do not create organization audit rows. CI and unattended agents use a separate REST-administered Bridge service identity; they must not copy a human's keychain credential.
...
The worker slice claims with leases, records attempts, completes successes, reschedules failures with capped exponential backoff and configurable proportional jitter, and dead-letters events at the configured budget. The deployable worker also runs scheduled assumption-expiry, overdue-blocker escalation, and configured email-digest cycles at bounded intervals before delivery polling; all use the maintenance boundary and emit safe completion/failure logs. Project administrators can inspect a project-scoped queue snapshot with status counts, total attempts, ready work, expired leases, oldest-ready age, and privacy-minimized per-channel delivery receipts. Failed or dead-letter events can be requeued with an optimistic attempt-count check; replay preserves the event ID for downstream idempotency, resets delivery state, and writes an audit event in the same transaction. Immediate email and Slack handlers pass stable event/channel idempotency keys to injected providers. Deferred email receipts are maintenance-claimed by due time with a recoverable lease, grouped by recipient/project, assigned a persisted stable digest batch key before send, and completed from one provider result; Slack persists its separate semantic dedupe key. The controlled-pilot email composition uses AWS SES v2 with the default AWS credential chain and a bounded secret-managed principal/address directory. An optional separate worker listener verifies signed SNS bounce/complaint callbacks and forwards privacy-minimized feedback through the canonical authenticated REST command. Live SES account/identity, HTTPS ingress, secret provisioning, and provider failure-window validation remain external deployment work.
...
4. The provider-neutral email handler resolves the recipient through an injected directory, reads the tenant-scoped human email preference from the repository when available, falls back to the directory's default, renders bounded plain text, and calls an injected sender without persisting the address. The deployable controlled-pilot adapter supplies a secret-managed principal/address mapping and AWS SES v2 sender; provider credentials come only from the standard AWS runtime chain.
...
6. `outbox_deliveries` records the destination hash, optional semantic dedupe key, preference outcome, attempt, delivery status, sanitized error, provider message ID, and—when an authenticated provider adapter reports one—normalized feedback type/provider/timestamp. Feedback is constrained to the channel's provider, is written through `POST /v1/projects/:projectId/integrations/notifications/delivery-feedback`, and causes the receipt to remain failed so workers do not automatically retry it. The in-app notification remains the canonical human read model.
7. When explicitly enabled, the worker's separate `POST /webhooks/aws/ses` listener bounds the raw request, requires the SNS header/envelope type pair and an exact topic-to-project mapping, validates an SNS-owned HTTPS certificate URL and certificate lifetime, verifies the version 1 or 2 RSA signature, and parses only SES bounce/complaint message ID, type, and timestamp. It sends the normalized command to the existing REST endpoint with a project-restricted `integration` service token holding `bridge:notifications:write`; it does not call a repository or database adapter. Subscription confirmation is separately opt-in and follows only a verified same-SNS-host URL whose topic and token match the signed envelope. A bounded process-local SNS message-ID cache reduces retries, while the REST command remains the durable idempotency boundary.
8. When `email` or `all` delivery is enabled, the worker runtime schedules the same SES sender/directory for the email-digest cycle at a bounded interval. Digest receipts persist only due/lease times, destination hashes, and batch keys; recipient addresses remain in runtime memory and deployment secret storage rather than the outbox.

Protected-review email bypasses muted/digest preferences in the current policy seam. Ordinary notifications support immediate delivery, explicit suppression, or durable digest batching. Human email preferences are managed through `GET/POST /v1/notifications/preferences`; the provider-neutral scheduler/delivery cycle and SES controlled-pilot composition are implemented. A trusted integration can submit already-normalized provider metadata through the REST boundary, and the optional SES/SNS listener now supplies that adapter for signed bounce/complaint callbacks. The application still verifies project access, matches the provider message ID inside the tenant transaction, records a controlled audit event, and exposes the signal in support/outbox reads. Slack webhook/signature ingress, live SES/SNS routing and account setup, live links, IAM/secret provisioning, and failure-window evidence remain deployment work. Slack remains a project-wide channel rather than a per-user preference surface.
...
The SES/SNS ingress implements the repository-owned signed-webhook subset with strict route/method/body
bounds, exact topic mapping, an SNS-only HTTPS certificate allowlist, no certificate redirects,
certificate validity checks, RSA/SHA-1 and RSA/SHA-256 verification, controlled subscription URL
validation, and bounded process-local replay state. Raw provider messages, recipient addresses,
certificate/subscription URLs, signatures, and service tokens are not logged or persisted. The
listener returns retryable failure when the canonical API or signing certificate is unavailable and
uses a project-restricted, revocable Bridge service identity for REST forwarding. Deployment still
owns public TLS/routing, distributed edge rate limits, provider account/topic policy, secret rotation,
and live adversarial/retry evidence.
...
`@bridge/observability` now implements a dependency-free, process-local metrics registry with fixed recording methods and Prometheus text rendering. Standalone API/MCP runtimes share one registry with the application and PostgreSQL repository and expose `GET /metrics`; the worker exposes its injected registry from a separate loopback-default HTTP listener. In-memory test/runtime paths use the same transaction instrumentation. Durable stores place top-level repository and maintenance transactions behind a FIFO admission boundary aligned with the configured Postgres.js connection maximum; nested transactions reuse the admitted slot. Outbox, email, and Slack handlers accept the registry explicitly, preserving the worker/integration boundary. Labels exclude tenant, project, principal, record, and content dimensions; HTTP operations are bounded route names, unmatched paths collapse to one label, and a 128-operation process budget collapses excess values to `overflow`.

The implemented portable subset covers HTTP request/outcome/duration and `401`/`403` denials, bounded API/MCP authentication outcomes and failure flows, bounded MCP initialize and tool-call outcome/duration, repository transaction outcome/duration, top-level PostgreSQL transaction admission capacity/in-use/waiting gauges and wait duration, context outcome/duration/candidate/result counts, outbox processing/retry/dead-letter and oldest-claimed age, email/Slack handling outcomes/duration, fixed-vocabulary idempotent write outcomes, and application conflict totals. Admission pressure is a process-level lower boundary: non-transactional health/bootstrap/identity queries and PostgreSQL/RDS server connection state remain outside it. The selected deployment/provider must still supply server/pool saturation telemetry rather than relying on unstable driver internals.
...
`POST /v1/projects/:projectId/adapter-diagnostics` is the canonical write path for a client adapter to record a bounded doctor result: client, capabilities, MCP state, check names/statuses, reporter envelope, correlation ID, and observation time. PostgreSQL keeps the latest result per project/client plus a capped history of the prior 20 report summaries behind tenant RLS; history stores only status, MCP state, check totals, failed check names, and observation time. It does not store URLs, check details, repository content, secrets, or an approval decision. `GET /v1/admin/projects/:projectId/support` reuses the application repository boundary to report active unrouted questions, overdue protected decisions, active assumptions due within seven days, runs waiting for human input with remaining blocking-question counts, dead-letter/pending delivery counts, recorded agent client/capability observations, and the latest diagnostic summaries with local trend metadata. The application derives bounded check totals, failed check names, observation counts, healthy counts, and latest status transitions from stored report metadata, and the web **Support** view presents them as responsive adapter cards with the earlier history behind an accessible disclosure. Each support signal links back to the canonical Assumptions, Agent Runs, Questions, Decisions, or Outbox view without exposing assumption statements or run task summaries. Persisted doctor metadata and trends are operational observations, not provider connectivity proof, time-series telemetry, automatic remediation, or human approval.
...
The local development mode needs no infrastructure when `DATABASE_URL` is absent and uses seeded in-memory state plus the fixed reviewer switcher. The repository also provides `infra/containers/compose.yaml` and the `pnpm services:up|down|status|reset` operator commands for a loopback-only PostgreSQL/MinIO stack. PostgreSQL uses host port `5433` by default and creates the `bridge` development database, an isolated `bridge_test` database, and fixed local-only service roles; the Compose reset command requires `--confirm` and removes only those named local volumes. For durable development, an operator runs `DATABASE_URL=<bootstrap-url> pnpm db:migrate` and the existing role-reconciliation script explicitly, then configures the API runtime URL on the non-superuser `bridge_runtime` role. Current artifact bodies remain in PostgreSQL, so the local MinIO service is an S3-compatible compatibility surface for future storage work rather than an implicit production object-storage claim. For durable OIDC development, configure the issuer/audience/client/session variables documented in `docs/authentication.md`. Production mode fails startup without OIDC; the fixed principal header is development-only.
...
Repository validation now combines a real Biome formatter/linter with the dependency-free architecture gates: `pnpm check` runs changed-file formatting and linting against `main`, full-repository whitespace checks, package-boundary and direct-import policy checks, generated Zod request-schema compatibility, reviewed REST/MCP response-contract and transport-surface baselines, and the high-confidence secret gate before workspace checks. CI also runs `pnpm dependency:check`, which adds a production dependency audit. `config/package-boundaries.json` and `config/dependency-policy.json` make workspace direction, direct dependency declarations, and forbidden transport/infrastructure imports executable. `config/contract-schema-baseline.json` snapshots every exported Zod contract as draft-2020-12 JSON Schema, while `config/transport-contract-baseline.json` records the reviewed REST/MCP request and response contract categories. Contract changes must use the explicit `pnpm contracts:baseline:update` command in the same reviewed change; the gate never mutates baselines during ordinary checks.
