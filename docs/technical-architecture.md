...
Protected-review email bypasses muted/digest preferences in the current policy seam. Ordinary notifications support immediate delivery, explicit suppression, or durable digest batching. Human email preferences are managed through `GET/POST /v1/notifications/preferences`; the provider-neutral scheduler/delivery cycle and SES controlled-pilot composition are implemented. A trusted integration can submit already-normalized SES bounce/complaint or Slack provider-failure metadata through the REST boundary; the application verifies project access, matches the provider message ID inside the tenant transaction, records a controlled audit event, and exposes the signal in support/outbox reads. SES/Slack webhook signature verification, provider-specific ingress wiring, account/identity setup, live links, IAM/deployment validation, and failure-window evidence remain deployment work. Slack remains a project-wide channel rather than a per-user preference surface.
