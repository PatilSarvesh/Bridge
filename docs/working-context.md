...
6. Forward-only migrations `0033_sparkling_carlie_cooper.sql` and `0034_mute_energizer.sql` allow question-less/source-less decisions for assumption provenance with an explicit source-shape check; `0035_odd_gravity.sql` extends the notification type constraint for assumption expiry. Mapper, application, REST, worker, web, and migration regressions cover creation/linking, human-only authority, scheduled idempotence, notification fanout, filtering, and delivery scheduling. No database command was run; PostgreSQL integration remains gated by an explicitly isolated `BRIDGE_TEST_DATABASE_URL`.
