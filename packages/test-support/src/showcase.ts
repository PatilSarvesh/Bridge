...
function artifactVersion(
  input: Omit<ArtifactVersion, "approvalStatus" | "contentSha256">,
): ArtifactVersion {
    ...
    if (!await scopedRepository.getProjectOwnershipConfiguration(project.id)) {
      const ownership: ProjectOwnershipConfiguration = {
        ...
        roles: [
          ...
          { name: "architecture-reviewer", description: "Reviews system boundaries, compatibility, and maintainability." },
    ...
    const runs: readonly AgentRun[] = [
      {
        ...
        scope: { repository: "payments-api", component: "webhooks", branch: "feature/signed-webhooks", environment: "production", workItem: "PAY-284" },
      ...
      },
      {
        ...
        scope: { repository: "payments-api", component: "event-store", branch: "feature/retention", workItem: "PAY-231" },
        status: "completed",
      ...
      },
      {
        ...
        scope: { repository: "bridge", component: "integrations", branch: "spike/incident-sync", workItem: "BRG-139" },
        status: "failed",
        contextSnapshotIds: [],
        questionIds: [],
        artifactVersionIds: [],
    ...
    const questions: readonly Question[] = [
      {
        ...
        type: "approval",
        category: "release",
        context: "The new webhook path verifies signatures, deduplicates delivery attempts, and has passed staging load tests. Production enablement changes the external delivery boundary.",
        whyItMatters: "A rollout without explicit security approval could expose customer events or cause duplicate partner actions.",
        risk: "protected",
        ...
        options: [
          { key: "staged-rollout", label: "Approve a staged 10% rollout", tradeoffs: "Limits initial exposure but requires active monitoring and a second promotion step." },
          { key: "full-rollout", label: "Approve full production rollout", tradeoffs: "Finishes sooner but increases the impact of an undiscovered delivery issue." },
          { key: "hold", label: "Hold for additional evidence", tradeoffs: "Reduces immediate risk but delays partner onboarding." },
        ],
        relatedLinks: [
          { type: "repository", label: "payments-api repository", url: "https://github.com/bridge-labs/payments-api" },
          { type: "work_item", label: "PAY-284 rollout issue", url: "https://github.com/bridge-labs/payments-api/issues/284" },
          { type: "branch", label: "signed-webhooks branch", url: "https://github.com/bridge-labs/payments-api/tree/feature/signed-webhooks" },
        ...
        responses: [{
          ...
          answer: "Start with a staged 10% rollout and automatically pause when signature failures exceed the agreed threshold.",
          rationale: "Staging preserves the rollback path while producing production evidence before full enablement.",
      ...
      },
      {
        ...
        type: "decision",
        category: "architecture",
        context: "Partner endpoints occasionally return transient 429 and 503 responses. The retry worker currently has no agreed maximum delivery window.",
        whyItMatters: "The choice affects partner expectations, queue growth, support load, and how quickly permanent failures become visible.",
        risk: "high",
        policyAction: "block",
        policyVersion: 1,
        ...
        options: [
          { key: "six-hours", label: "Retry for up to 6 hours", tradeoffs: "Keeps the queue small but may abandon recoverable partner outages." },
          { key: "twenty-four-hours", label: "Retry for up to 24 hours", tradeoffs: "Improves delivery durability at the cost of a larger queue and later failure visibility." },
          { key: "seventy-two-hours", label: "Retry for up to 72 hours", tradeoffs: "Maximizes eventual delivery but creates significant operational backlog." },
        ],
        relatedLinks: [{ type: "external", label: "Webhook reliability guidance", url: "https://example.com/webhook-reliability" }],
        ...
        responses: [{
          ...
          rationale: "Most partner incidents recover within a day and the queue capacity model supports that window.",
      ...
      },
      {
        ...
        context: "The event store currently retains delivery history indefinitely even though support workflows use only recent records.",
        whyItMatters: "A clear retention period controls storage growth and gives support and privacy teams a shared expectation.",
        risk: "medium",
        ...
        options: [
          { key: "ninety-days", label: "Retain for 90 days", tradeoffs: "Lowest storage cost but a shorter support investigation window." },
          { key: "one-year", label: "Retain for one year", tradeoffs: "Balances support history with bounded storage and deletion operations." },
          { key: "indefinite", label: "Retain indefinitely", tradeoffs: "Maximum history with unbounded storage and privacy obligations." },
        ],
        recommendationKey: "one-year",
        fallback: "Retain for 90 days if no human decision is recorded before implementation starts.",
        scope: { repository: "payments-api", component: "event-store", environment: "production", workItem: "PAY-231" },
        createdById: agent.id,
        createdByType: agent.type,
        createdAt: shiftedIso(anchor, -15),
        status: "accepted",
        responses: [{
          ...
          rationale: "One year covers the contractual dispute window without preserving operational records indefinitely.",
          optionKey: "one-year",
      ...
      },
      {
        ...
        context: "Enterprise customers require event metadata to remain within an approved region while the reporting service is being expanded.",
        whyItMatters: "The storage region is a protected compliance boundary and must be approved by accountable humans.",
        risk: "protected",
        policyAction: "protected_approval",
        policyVersion: 1,
        ...
        options: [
          { key: "eu-only", label: "Store regulated metadata in the EU region only", tradeoffs: "Meets current customer commitments but adds region-aware routing." },
          { key: "global", label: "Use the global event store", tradeoffs: "Simpler operations but does not meet the recorded residency requirement." },
        ],
        recommendationKey: "eu-only",
        fallback: null,
        scope: { repository: "payments-api", component: "event-store", environment: "production", workItem: "PAY-198" },
        createdById: agent.id,
        createdByType: agent.type,
        createdAt: shiftedIso(anchor, -35),
        status: "accepted",
        responses: [{
          ...
          rationale: "This matches the approved customer and privacy boundary while keeping operational data minimal.",
          optionKey: "eu-only",
      ...
      },
      {
        ...
        context: "The redesigned workspace introduces new guidance across inbox, specification, and support empty states.",
        whyItMatters: "A named owner is needed so terminology remains consistent and understandable across every workspace area.",
        risk: "low",
        policyAction: "ask_async",
        policyVersion: 1,
      ...
      },
      {
        ...
        type: "decision",
        category: "product",
        context: "A second agent run asked the same currency question after an earlier question had already been routed.",
        whyItMatters: "Showing duplicate history verifies that Bridge prevents repeated interruptions without hiding provenance.",
        risk: "low",
        policyAction: "ask_async",
        policyVersion: 1,
        ...
        options: [
          { key: "inr", label: "Indian rupee", tradeoffs: "Fits the first local demo but is less neutral for a reusable sample." },
          { key: "user-selected", label: "Require the user to choose", tradeoffs: "Avoids a default but adds setup friction." },
        ],
      ...
      },
      {
        ...
        options: [
          { key: "retain", label: "Retain the old copy", tradeoffs: "Reduces change but carries obsolete terminology." },
          { key: "replace", label: "Replace it with the new language", tradeoffs: "Improves consistency but expands the experiment." },
        ],
    ...
    const decisions: readonly Decision[] = [
      {
        ...
        rationale: "A one-year window covers support and contractual needs while keeping storage and deletion obligations bounded.",
        category: "data-retention",
        scope: { repository: "payments-api", component: "event-store", environment: "production", workItem: "PAY-231" },
    ...
    const assumptions: readonly Assumption[] = [
      {
        ...
        rationale: "Current partner onboarding schedules and staging measurements support this temporary rollout estimate.",
        category: "capacity",
        risk: "low",
        confidence: "medium",
        reversible: true,
      ...
      },
      {
        ...
        rationale: "The current pilot group primarily reviews Bridge from managed laptops, while mobile remains supported.",
        category: "design",
        risk: "low",
        confidence: "medium",
        reversible: true,
        reversalCost: "Reprioritize responsive density and navigation testing if mobile usage is higher than expected.",
      ...
      },
      {
        ...
        rationale: "The initial product plan used this premise while the retention owner gathered contractual evidence.",
        category: "data-retention",
        risk: "low",
        confidence: "high",
        reversible: true,
      ...
      },
      {
        ...
        rationale: "Early discovery included only EU customers and temporarily treated that cohort as representative.",
        category: "product",
        risk: "low",
        confidence: "low",
        reversible: true,
...
`,
    ...
    const existingSnapshots = new Set((await scopedRepository.listContextSnapshots(project.id)).map((snapshot) => snapshot.id));
    ...
    for (const notification of notifications) {
      if (!await scopedRepository.getNotification(notification.id)) await scopedRepository.saveNotification(notification);
    }
