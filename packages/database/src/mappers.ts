...
export function questionToRow(question: Question): typeof questions.$inferInsert {
  return {
    ...
    reviewerIds: question.reviewerIds,
    reviewerRoles: question.reviewerRoles,
    requiredReviewerRoles: question.requiredReviewerRoles,
    requiredReviewerQuorum: question.requiredReviewerQuorum ?? {},
...
export function questionFromRows(
  ...
  return {
    ...
    ...(Object.keys(row.requiredReviewerQuorum).length > 0
...
export function decisionToRow(decision: Decision): typeof decisions.$inferInsert {
  return {
    ...
    sourceResponseId: decision.sourceResponseId ?? null,
...
export function decisionFromRow(row: DecisionRow): Decision {
  return {
    ...
    scope: row.scope,
    ownerId: row.ownerId,
    ...(row.sourceResponseId === null ? {} : { sourceResponseId: row.sourceResponseId }),
    status: row.status,
    createdAt: row.createdAt,
