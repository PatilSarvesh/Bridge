...
async function bridgeFetch<T>(path: string, init?: RequestInit, actingPrincipalId = defaultPrincipalId): Promise<T> {
  ...
  if (!response.ok) {
    const message =
      typeof body === "object" && body !== null && "message" in body ? String(body.message) : "Bridge request failed.";
    throw new Error(message);
  }
  return body as T;
}

async function bridgeDownload(path: string, body: unknown, actingPrincipalId: string): Promise<void> {
  const response = await fetch(`${apiUrl}${path}`, {
    method: "POST",
    credentials: "include",
    headers: {
...
export default function Home() {
  ...
  const [decisionLifecycleStatus, setDecisionLifecycleStatus] = useState<"superseded" | "expired" | "revoked">(
    "revoked",
  );
  ...
  const [assumptionResolutionStatus, setAssumptionResolutionStatus] = useState<"confirmed" | "rejected" | "expired">(
    "confirmed",
  );
  ...
  const isOrganizationAdmin =
    activePrincipal?.roles.some((role) => normalizedRole(role) === "organization-admin") ?? false;
  const isProjectAdmin = isOrganizationAdmin || activeRoles.some((role) => normalizedRole(role) === "project-admin");
  ...
  const inboxFilterOptions = useMemo(
    () => ({
      categories: [...new Set(questions.map((question) => question.category))].sort((left, right) =>
        left.localeCompare(right),
      ),
      roles: [...new Set(questions.flatMap((question) => [...question.ownerRoles, ...question.reviewerRoles]))].sort(
  ...
  const hasDecisionFilters =
    Boolean(decisionFilters.includeHistory) ||
  ...
  const loadProjects = useCallback(async () => {
    ...
    setError(undefined);
    try {
      const response = await bridgeFetch<{ items: readonly Project[] }>("/v1/projects", undefined, activePrincipalId);
      setProjects(response.items);
      setSelectedProjectId((current) =>
        current && response.items.some((project) => project.id === current) ? current : response.items[0]?.id,
      );
    } catch (requestError) {
  ...
  const loadOwnership = useCallback(async () => {
    ...
    try {
      ...
      setError(
        requestError instanceof Error ? requestError.message : "Unable to load project ownership configuration.",
      );
    } finally {
      setOwnershipLoading(false);
    }
  ...
  const saveOwnership = async () => {
    ...
    try {
      ...
      setError(
        requestError instanceof Error ? requestError.message : "Unable to save project ownership configuration.",
      );
    } finally {
      setSubmitting(false);
    }
      ...
      roleList(String(form.get("policyReviewerQuorum") ?? ""))
        ...
        .filter(
          (entry): entry is [string, string] =>
            entry.length === 2 && Number.isInteger(Number(entry[1])) && Number(entry[1]) >= 1,
        )
  ...
  const loadArtifacts = useCallback(async () => {
    ...
    try {
      ...
      setSelectedArtifactId((current) =>
        current && response.items.some((artifact) => artifact.id === current) ? current : response.items[0]?.id,
      );
    } catch (requestError) {
  ...
  const loadNotificationPreference = useCallback(async () => {
    ...
    try {
      ...
      setNotificationPreference(response.items.find((item) => item.channel === "email")?.preference ?? "immediate");
    } catch (requestError) {
  ...
  const saveNotificationPreference = useCallback(
    async (preference: NotificationDeliveryPreference) => {
  ...
  const loadReferenceData = useCallback(async () => {
    ...
    try {
      ...
      if (decisionFilters.createdFrom)
        decisionParameters.set("createdFrom", `${decisionFilters.createdFrom}T00:00:00.000Z`);
      ...
      setSelectedRunId((current) =>
        current && runResponse.items.some((run) => run.id === current) ? current : runResponse.items[0]?.id,
      );
    } catch (requestError) {
  ...
  const loadAnalytics = useCallback(async () => {
    ...
    try {
      ...
      setAnalytics(
        await bridgeFetch<ProjectAnalytics>(
  ...
  const loadSupport = useCallback(async () => {
    ...
    try {
      setSupport(
        await bridgeFetch<ProjectSupport>(
  ...
  const auditParameters = useCallback(
  ...
  const loadAudit = useCallback(
    async (offset = 0) => {
      ...
      try {
        ...
        setAuditPage(
          await bridgeFetch<AuditPage>(`${path}?${auditParameters(offset).toString()}`, undefined, activePrincipalId),
        );
      } catch (requestError) {
        setAuditPage(undefined);
  ...
  const exportProjectData = useCallback(async () => {
    ...
    setError(undefined);
    try {
      await bridgeDownload(`/v1/admin/projects/${selectedProjectId}/export`, {}, activePrincipalId);
      await loadAudit(0);
    } catch (requestError) {
  ...
  const loadOrganizationMembers = useCallback(async () => {
    ...
    try {
      ...
      setSelectedMemberId((current) =>
        current && response.items.some((member) => member.id === current) ? current : response.items[0]?.id,
      );
    } catch (requestError) {
      setOrganizationMembers([]);
      setOrganizationProjects([]);
  ...
  const markNotificationRead = useCallback(
    async (notificationId: string) => {
      const updated = await bridgeFetch<Notification>(
  ...
  const markAllNotificationsRead = useCallback(async () => {
    await bridgeFetch<{ markedCount: number }>(
  ...
  const selectedArtifactReviewerIds =
    selectedArtifactVersion?.reviewerAssignment?.reviewerIds ?? selectedArtifact?.reviewerIds ?? [];
  ...
  const selectedDecisionConflicts = useMemo(
    () =>
      selectedDecision
        ? decisionConflicts.filter(
            (conflict) => conflict.left.id === selectedDecision.id || conflict.right.id === selectedDecision.id,
          )
        : [],
  ...
  const canResolveSelectedAssumption = Boolean(
    selectedAssumption && (selectedProject?.decisionOwnerIds.includes(activePrincipalId) || isProjectAdmin),
  );
  ...
  const loadQuestionAudienceView = async (mode: "explain" | "rewrite") => {
    ...
    try {
      ...
      setQuestionAudienceView(
        await bridgeFetch<QuestionAudienceView>(
      ...
      setQuestionAudienceView(undefined);
      setError(
        requestError instanceof Error ? requestError.message : "Unable to explain this question for the selected role.",
      );
    } finally {
  ...
  const proposeAnswer = async () => {
    ...
    try {
      await bridgeFetch(
        `/v1/questions/${selectedQuestion.id}/responses`,
        {
          method: "POST",
          body: JSON.stringify({
  ...
  const saveResponseEdit = async () => {
    ...
    try {
      await bridgeFetch(
        `/v1/questions/${selectedQuestion.id}/responses/${editingResponseId}`,
        {
          method: "PATCH",
          body: JSON.stringify({
  ...
  const reviewQuestion = async () => {
    ...
    try {
      await bridgeFetch(
        `/v1/questions/${selectedQuestion.id}/reviews`,
        {
          method: "POST",
          body: JSON.stringify({
            expectedVersion: selectedQuestion.version,
  ...
  const overrideQuestionApproval = async () => {
    ...
    try {
      await bridgeFetch(
        `/v1/questions/${selectedQuestion.id}/override`,
        {
          method: "POST",
          body: JSON.stringify({
            expectedVersion: selectedQuestion.version,
  ...
  const reassignQuestion = async (event: FormEvent<HTMLFormElement>) => {
    ...
    try {
      await bridgeFetch(
        `/v1/questions/${selectedQuestion.id}/assignments`,
        {
          method: "POST",
          body: JSON.stringify({
            expectedVersion: selectedQuestion.version,
  ...
  const addQuestionComment = async () => {
    ...
    try {
      await bridgeFetch(
        `/v1/questions/${selectedQuestion.id}/comments`,
        {
          method: "POST",
          body: JSON.stringify({
            expectedVersion: selectedQuestion.version,
  ...
  const saveCommentEdit = async () => {
    ...
    try {
      await bridgeFetch(
        `/v1/questions/${selectedQuestion.id}/comments/${editingCommentId}`,
        {
          method: "PATCH",
          body: JSON.stringify({
            expectedVersion: selectedQuestion.version,
  ...
  const requestQuestionClarification = async () => {
    if (!selectedQuestion || !selectedQuestion.canRequestClarification || clarificationReason.trim().length < 10)
      return;
    setSubmitting(true);
    setError(undefined);
    try {
      await bridgeFetch(
        `/v1/questions/${selectedQuestion.id}/clarification`,
        {
          method: "POST",
  ...
  const reopenQuestion = async () => {
    ...
    try {
      await bridgeFetch(
        `/v1/questions/${selectedQuestion.id}/reopen`,
        {
          method: "POST",
  ...
  const acceptDecision = async () => {
    ...
    try {
      await bridgeFetch(
        `/v1/questions/${selectedQuestion.id}/accept`,
        {
          method: "POST",
  ...
  const approveSpecification = async () => {
    ...
    try {
      await bridgeFetch(
        `/v1/artifact-versions/${selectedArtifactVersion.id}/approve`,
        {
          method: "POST",
  ...
  const reviewSpecification = async () => {
    ...
    try {
      await bridgeFetch(
        `/v1/artifact-versions/${selectedArtifactVersion.id}/reviews`,
        {
          method: "POST",
  ...
  const loadDecisionImpact = async () => {
    ...
    setError(undefined);
    try {
      setDecisionLifecycleImpact(
        await bridgeFetch<DecisionLifecycleImpact>(
  ...
  const resolveAssumption = async () => {
    ...
    setError(undefined);
    try {
      await bridgeFetch<Assumption>(
        `/v1/assumptions/${selectedAssumption.id}/resolve`,
        {
          method: "POST",
          body: JSON.stringify({
            ...
            ...(assumptionResolutionStatus === "confirmed" && assumptionCreateDecision ? { createDecision: true } : {}),
          }),
        },
        activePrincipalId,
      );
  ...
  const navigateTo = (destination: View) => {
    setView(destination);
    if (
      !administrationNavigation.some((item) => item.view === destination) ||
  ...
  const openNotification = async (notification: Notification) => {
    ...
    try {
      ...
      if (notification.targetType === "decision") {
        ...
        const artifact =
          notification.targetType === "artifact"
        ...
        setView("questions");
        const question =
          notification.targetType === "question"
  ...
  if (!authenticationReady) {
    return (
      <main className="auth-shell">
        <div className="auth-card">Loading Bridge…</div>
      </main>
    );
  }
    ...
    return (
      <main className="auth-shell">
        <section className="auth-card" aria-labelledby="bridge-sign-in-title">
          ...
          <p>
            Your identity provider verifies you. Bridge then applies active organization and project memberships on
            every request.
          </p>
          {error ? (
          ...
          <a className="auth-action" href={authentication.loginUrl}>
            Continue with SSO
          </a>
      ...
      <aside className="sidebar">
        <div className="brand">
          <span className="brand-mark">
            <BridgeIcon name="bridge" size={19} />
          </span>
        ...
        <div className="project">
          <label htmlFor="bridge-project">
            <BridgeIcon name="repositories" size={15} />
          ...
          <div className="nav-group nav-primary">
            {primaryNavigation.map((item) => (
              ...
              >
                ...
                {navigationCounts[item.view] !== undefined ? (
                  <span className="nav-badge">{navigationCounts[item.view]}</span>
                ) : null}
              </button>
            ))}
          </div>
        ...
        <div className="identity">
          ...
          {authentication?.mode === "development" ? (
            <div className="reviewer">
              <label htmlFor="bridge-reviewer">
                <small>Preview authority as</small>
              </label>
              <select
                id="bridge-reviewer"
        ...
        <header className="topbar">
          ...
          <div className="topbar-actions">
            <span className="topbar-project">
              <BridgeIcon name="bridge" size={15} />
              {view === "guide"
            ...
            <span className="topbar-avatar" title={activePrincipal?.displayName}>
              {displayInitials(activePrincipal?.displayName)}
            </span>
          </div>
        </header>
        <div className="content">
          ...
          {view === "guide" ? (
            ...
            <>
              <div className="title-row">
                <div>
                  ...
                  <p>
                    Operator-only project health derived from routing, decision review, durable delivery, and recorded
                    agent runs.
                  </p>
                </div>
                <button
              ...
              {!supportLoading && !support ? (
                <div className="empty">Select a project to load support signals.</div>
              ) : null}
              {!supportLoading && support ? (
                <div className="support-stack">
                  ...
                  <section className="analytics-panel support-panel">
                    <div className="analytics-panel-heading">
                      <div>
                        <h2>Unresolved routing</h2>
                        <p>
                          Questions without a direct owner or role assignment remain visible for administrator action.
                        </p>
                      </div>
                      <small>{support.routing.unroutedQuestions.length} items</small>
                    </div>
                    {support.routing.unroutedQuestions.length === 0 ? (
                      <div className="empty">No active questions are currently unrouted.</div>
                    ) : (
                        ...
                        {support.routing.unroutedQuestions.map((question) => (
                          ...
                          >
                            ...
                            <span>
                              ...
                              <small>
                                {question.category} · {question.status.replaceAll("_", " ")} ·{" "}
                  ...
                  <section className="analytics-panel support-panel">
                    <div className="analytics-panel-heading">
                      <div>
                        <h2>Overdue protected decisions</h2>
                        <p>Protected decisions past their review date need an explicit human lifecycle decision.</p>
                      </div>
                      <small>{support.decisions.overdueProtected.length} items</small>
                    </div>
                    {support.decisions.overdueProtected.length === 0 ? (
                      <div className="empty">No protected decisions are overdue.</div>
                    ) : (
                      <div className="support-list">
                        {support.decisions.overdueProtected.map((decision) => (
                          ...
                          >
                            ...
                            <span>
                              <strong>{decision.category} decision</strong>
                              <small>
                                Owner {displayIdentityName(decision.ownerId, principals)} · review due{" "}
                                {new Date(decision.reviewAt).toLocaleDateString()}
                              </small>
                            </span>
                  ...
                  <section className="analytics-panel support-panel">
                    <div className="analytics-panel-heading">
                      <div>
                        <h2>Assumptions nearing expiry</h2>
                        <p>
                          Active assumptions due within seven days are listed without exposing their governed statement;
                          overdue items may indicate a delayed maintenance cycle.
                        </p>
                      </div>
                      <small>{support.assumptions.expiring.length} items</small>
                    </div>
                    {support.assumptions.expiring.length === 0 ? (
                      <div className="empty">No active assumptions are due within seven days.</div>
                    ) : (
                      <div className="support-list">
                        {support.assumptions.expiring.map((assumption) => (
                          ...
                          >
                            ...
                            <span>
                              <strong>{assumption.category} assumption</strong>
                              <small>
                                {assumption.overdue
                                  ? "overdue"
                                  : `expires ${new Date(assumption.expiresAt).toLocaleDateString()}`}{" "}
                                · {assumption.confidence} confidence · created by{" "}
                                {displayIdentityName(assumption.createdById, principals)}
                              </small>
                            </span>
                            <span className={assumption.overdue ? "status status-rejected" : "status"}>
                              {assumption.overdue ? "overdue" : "expiring"}
                            </span>
                          </button>
                        ))}
                      </div>
                  ...
                  <section className="analytics-panel support-panel">
                    <div className="analytics-panel-heading">
                      <div>
                        <h2>Runs waiting for human input</h2>
                        <p>
                          These runs remain blocked until their linked questions are resolved; open a run to inspect its
                          governed handoff.
                        </p>
                      </div>
                      <small>{support.runs.blocked.length} items</small>
                    </div>
                    {support.runs.blocked.length === 0 ? (
                      <div className="empty">No runs are currently waiting for human input.</div>
                    ) : (
                      <div className="support-list">
                        {support.runs.blocked.map((run) => (
                          ...
                          >
                            ...
                            <span>
                              ...
                              <small title={run.id}>
                                {run.client.replaceAll("_", " ")} · {run.capability.replaceAll("_", " ")} ·{" "}
                                {run.remainingBlockingQuestionCount} blocking question
                                {run.remainingBlockingQuestionCount === 1 ? "" : "s"} · updated{" "}
                  ...
                  <section className="analytics-panel support-panel">
                    <div className="analytics-panel-heading">
                      <div>
                        ...
                        <p>
                          Dead letters contain only operational identifiers here; inspect and replay through the
                          existing project-admin outbox controls.
                        </p>
                      </div>
                    ...
                    <div className="support-delivery-summary" role="status">
                      <strong>
                        {support.delivery.providerFeedbackCount === 0
                          ? "No provider feedback recorded"
                          : `${support.delivery.providerFeedbackCount} provider feedback signal${support.delivery.providerFeedbackCount === 1 ? "" : "s"}`}
                      </strong>
                      <span>
                        {support.delivery.providerFeedbackCount === 0
                          ? "Delivery receipts have not reported a bounce, complaint, or provider failure."
                          : "Feedback is attached to the matching receipt and prevents another automatic send."}
                      </span>
                    </div>
                    {support.delivery.providerFeedback.length > 0 && (
                      <div className="analytics-table-wrap">
                        <table className="analytics-table support-table">
                          ...
                          <tbody>
                            {support.delivery.providerFeedback.map((feedback) => (
                              <tr key={feedback.deliveryId}>
                                <td>
                                  <strong>{feedback.channel}</strong>
                                  <small>{feedback.provider.toUpperCase()}</small>
                                </td>
                                <td>{feedback.type.replaceAll("_", " ")}</td>
                                <td>
                                  <time dateTime={feedback.receivedAt}>
                                    {new Date(feedback.receivedAt).toLocaleString()}
                                  </time>
                                </td>
                                <td>
                                  <code>{feedback.deliveryId}</code>
                                </td>
                              </tr>
                            ))}
                          </tbody>
                    ...
                    {support.delivery.deadLetterEvents.length === 0 ? (
                      <div className="empty">No dead-letter jobs are currently recorded.</div>
                    ) : (
                      <div className="analytics-table-wrap">
                        <table className="analytics-table support-table">
                          <thead>
                          ...
                          <tbody>
                            {support.delivery.deadLetterEvents.map((event) => (
                              <tr key={event.id}>
                                <td>
                                  <code>{event.id}</code>
                                </td>
                  ...
                  <section className="analytics-panel support-panel">
                    <div className="analytics-panel-heading">
                      <div>
                        <h2>Agent adapter observations</h2>
                        <p>{support.adapters.note}</p>
                      </div>
                      <small>
                        {support.adapters.mcpDiagnostics === "observed_from_runs"
                          ? "MCP observed from runs"
                          : support.adapters.mcpDiagnostics === "observed_from_doctor"
                            ? "Doctor reported"
                            : "MCP not reported"}
                      </small>
                    </div>
                    {support.adapters.items.length === 0 ? (
                      <div className="empty">No agent runs have reported adapter capability yet.</div>
                    ) : (
                      <div className="analytics-table-wrap">
                        <table className="analytics-table support-table">
                          <thead>
                            <tr>
                              ...
                              <th>Runs</th>
                              <th>Last observed</th>
                              <th>Last completed MCP run</th>
                            </tr>
                          </thead>
                          <tbody>
                            {support.adapters.items.map((adapter) => (
                              <tr key={adapter.client}>
                                <td>
                                  <strong>{adapter.client.replaceAll("_", " ")}</strong>
                                </td>
                                <td>{adapter.capabilities.join(", ")}</td>
                                <td>{adapter.runCount}</td>
                                <td>
                                  {adapter.lastObservedAt
                                    ? new Date(adapter.lastObservedAt).toLocaleString()
                                    : "Not available"}
                                </td>
                                <td>
                                  {adapter.lastSuccessfulMcpRunAt
                                    ? new Date(adapter.lastSuccessfulMcpRunAt).toLocaleString()
                                    : "Not observed"}
                                </td>
                              </tr>
                            ))}
                    ...
                    <p className="support-note">
                      Run metadata is derived from agent runs; repository-specific checks are recorded separately by{" "}
                      <code>bridge doctor</code>.
                    </p>
                  </section>
                  ...
                  <section className="analytics-panel support-panel">
                    <div className="analytics-panel-heading">
                      <div>
                        ...
                        <p>
                          Latest bounded <code>bridge doctor</code> results are stored per adapter, with a compact
                          recent history and local trend. Review the health summary and failed check names here; Bridge
                          never stores URLs, errors, or repository content.
                        </p>
                      </div>
                      <small>
                        {support.diagnostics.filter((diagnostic) => diagnostic.status === "pass").length} of{" "}
                        {support.diagnostics.length} healthy
                      </small>
                    </div>
                    {support.diagnostics.length === 0 ? (
                      <div className="empty">No bridge doctor reports are recorded.</div>
                    ) : (
                      <div className="support-diagnostic-grid" aria-label="Repository diagnostics by adapter">
                        {support.diagnostics.map((diagnostic) => (
                          <article
                            className={`support-diagnostic-card support-diagnostic-card-${diagnostic.status}`}
                            key={diagnostic.client}
                          >
                            <div className="support-diagnostic-heading">
                              ...
                              <span
                                className={
                                  diagnostic.status === "pass" ? "status status-approved" : "status status-rejected"
                                }
                              >
                            ...
                            <dl className="support-diagnostic-meta">
                              <div>
                                <dt>MCP</dt>
                                <dd>{diagnostic.mcpStatus.replaceAll("_", " ")}</dd>
                              </div>
                              <div>
                                <dt>Checks</dt>
                                <dd>
                                  {diagnostic.passedCheckCount} / {diagnostic.checkCount} passing
                                </dd>
                              </div>
                              <div>
                                <dt>Observed</dt>
                                <dd>
                                  <time dateTime={diagnostic.observedAt}>
                                    {new Date(diagnostic.observedAt).toLocaleString()}
                                  </time>
                                </dd>
                              </div>
                              <div>
                                <dt>Trend</dt>
                                <dd className={`support-diagnostic-trend-${diagnostic.trend.direction}`}>
                                  {diagnostic.trend.direction.replaceAll("_", " ")}
                                </dd>
                              </div>
                            </dl>
                            <div
                              className={`support-diagnostic-checks ${diagnostic.failingCheckNames.length > 0 ? "is-failing" : "is-passing"}`}
                            >
                              {diagnostic.failingCheckNames.length > 0 ? (
                                <>
                                  <strong>Needs attention</strong>
                                  <ul className="support-diagnostic-check-list">
                                    {diagnostic.failingCheckNames.map((name) => (
                                      <li key={name}>{name.replaceAll("-", " ")}</li>
                                    ))}
                                  </ul>
                                </>
                              ) : (
                                <>
                                  ...
                                  <span>
                                    {diagnostic.checkCount} check{diagnostic.checkCount === 1 ? "" : "s"} verified
                                  </span>
                                </>
                              )}
                            </div>
                            ...
                            <details className="support-diagnostic-history">
                              <summary>
                                Recent history{" "}
                                <span>
                                  {diagnostic.history.length} prior · {diagnostic.trend.observationCount} total ·{" "}
                                  {diagnostic.trend.healthyObservationCount} healthy
                                </span>
                              </summary>
                                ...
                                <ol>
                                  {[...diagnostic.history].reverse().map((observation, index) => (
                                    <li key={`${observation.observedAt}-${observation.status}-${index}`}>
                                      <span
                                        className={`support-diagnostic-history-status support-diagnostic-history-status-${observation.status}`}
                                      >
                                        {observation.status === "pass" ? "Healthy" : "Needs attention"}
                                      </span>
                                      <span>
                                        {observation.passedCheckCount} / {observation.checkCount} checks passing
                                      </span>
            ...
            <>
              <div className="title-row">
                <div>
                  ...
                  <p>
                    Manage repository metadata for this project. Bridge stores the canonical link and does not fetch
                    repository source.
                  </p>
                </div>
                <button
              ...
              <details className="panel-disclosure">
                <summary>
                  <span>
                    ...
                    <small>Add canonical metadata without fetching source.</small>
                  </span>
                  <em>Administrator action</em>
                </summary>
                <form
                ...
                >
                  <div className="member-form-grid">
                    ...
                    <label>
                      ...
                      <input
                        ...
                        placeholder="https://github.com/bridge-org/bridge"
                        required
                      />
                    </label>
                  </div>
                  <div className="member-form-actions">
                    <small>
                      Provider connectivity and source synchronization remain outside this metadata-only workflow.
                    </small>
                    <button className="primary" type="submit" disabled={submitting || !selectedProjectId}>
                      Link repository
                    </button>
                  </div>
                </form>
              ...
              <section className="organization-panel">
                <div className="organization-panel-heading">
                  <div>
                    ...
                    <p>Repository identity is unique within the organization and provider scope.</p>
                  </div>
                ...
                {!repositoriesLoading && repositories.length === 0 ? (
                  <div className="empty">No repositories are linked to this project.</div>
                ) : null}
                {!repositoriesLoading && repositories.length > 0 ? (
                  <div className="analytics-table-wrap">
            ...
            <>
              <div className="title-row">
                <div>
                  ...
                  <p>
                    Define project roles, human teams, and explainable owner/reviewer rules. Changes are saved as one
                ...
                <div className="member-form-actions">
                  ...
                  <button
                    ...
                    disabled={
                      ...
                      !ownershipDraft ||
                      JSON.stringify(ownershipConfiguration) === JSON.stringify(ownershipDraft)
                    }
              ...
              {!ownershipLoading && ownershipDraft ? (
                <div className="organization-stack">
                  <section className="organization-panel">
                    <div className="organization-panel-heading">
                      <div>
                        ...
                        <p>Role names are normalized when saved. Member assignment remains in Organization access.</p>
                      </div>
                    ...
                    <details className="inline-create-disclosure">
                      ...
                      <form className="member-form-grid" onSubmit={addOwnershipRole}>
                        <label>
                          Role name
                          <input name="roleName" placeholder="QA Lead" required minLength={2} />
                        </label>
                        <label>
                          Description
                          <input
                            name="roleDescription"
                            placeholder="Owns quality and release-readiness decisions"
                            required
                            minLength={2}
                          />
                        </label>
                    ...
                    {ownershipDraft.roles.length === 0 ? (
                      <div className="empty">No custom project roles are defined.</div>
                    ) : (
                      <div className="analytics-table-wrap">
                        <table className="analytics-table">
                          <thead>
                                ...
                                <td>
                                  <button
                                    ...
                                    onClick={() =>
                                      setOwnershipDraft({
                                        ...ownershipDraft,
                                        roles: ownershipDraft.roles.filter((candidate) => candidate !== role),
                                      })
                                    }
                                  >
                                    Remove
                  ...
                  <section className="organization-panel">
                    <div className="organization-panel-heading">
                      <div>
                        <h2>Human teams</h2>
                        <p>
                          Teams contain active human members with access to this project; agents cannot satisfy human
                          ownership.
                        </p>
                      </div>
                    ...
                    <details className="inline-create-disclosure">
                      ...
                      <form className="member-form-grid" onSubmit={addOwnershipTeam}>
                        <label>
                          Team key
                          <input name="teamKey" placeholder="quality" required pattern="[a-z0-9][a-z0-9-]*" />
                        </label>
                        <label>
                          Team name
                          <input name="teamName" placeholder="Quality" required />
                        </label>
                        <label>
                          Member IDs
                          <input name="teamMembers" placeholder="usr_qa_lead, usr_architect" required />
                        </label>
                        <div className="member-form-actions">
                          <small>
                            Available humans: {principals.map((principal) => principal.id).join(", ") || "none"}
                          </small>
                          <button className="secondary" type="submit">
                            Add team
                          </button>
                    ...
                    {ownershipDraft.teams.length === 0 ? (
                      <div className="empty">No project teams are configured.</div>
                    ) : (
                      <div className="analytics-table-wrap">
                        <table className="analytics-table">
                          <thead>
                                ...
                                <td>
                                  <button
                                    ...
                                    onClick={() =>
                                      setOwnershipDraft({
                                        ...ownershipDraft,
                                        teams: ownershipDraft.teams.filter((candidate) => candidate !== team),
                                      })
                                    }
                                  >
                                    Remove
                  ...
                  <section className="organization-panel">
                    <div className="organization-panel-heading">
                      <div>
                        ...
                        <p>
                          An empty match applies project-wide. Equal-priority overlapping owner or reviewer rules are
                          rejected.
                        </p>
                      </div>
                    ...
                    <details className="inline-create-disclosure">
                      ...
                      <form className="member-form-grid" onSubmit={addOwnershipRule}>
                        <label>
                          Rule key
                          <input name="ruleKey" placeholder="transfer-quality" required pattern="[a-z0-9][a-z0-9-]*" />
                        </label>
                        <label>
                          Rule name
                          <input name="ruleName" placeholder="Transfer quality ownership" required />
                        </label>
                        <label>
                          Priority
                          <input name="rulePriority" type="number" min={1} max={1000} defaultValue={100} required />
                        </label>
                        <label>
                          Category
                          <input name="ruleCategory" placeholder="quality" />
                        </label>
                        <label>
                          Repository
                          <input name="ruleRepository" placeholder="payments-api" />
                        </label>
                        <label>
                          Component
                          <input name="ruleComponent" placeholder="transfers" />
                        </label>
                        <label>
                          Owner member IDs
                          <input name="ownerPrincipalIds" placeholder="usr_architect" />
                        </label>
                        <label>
                          Owner roles
                          <input name="ownerRoles" placeholder="qa-lead" />
                        </label>
                        <label>
                          Owner team keys
                          <input name="ownerTeamKeys" placeholder="quality" />
                        </label>
                        <label>
                          Reviewer member IDs
                          <input name="reviewerPrincipalIds" placeholder="usr_security_reviewer" />
                        </label>
                        <label>
                          Reviewer roles
                          <input name="reviewerRoles" placeholder="architecture-reviewer" />
                        </label>
                        <label>
                          Reviewer team keys
                          <input name="reviewerTeamKeys" placeholder="architecture" />
                        </label>
                        <div className="member-form-actions">
                          <small>Use comma-separated values. Configure at least one owner or reviewer target.</small>
                          <button className="secondary" type="submit">
                            Add rule
                          </button>
                        </div>
                    ...
                    {ownershipDraft.rules.length === 0 ? (
                      <div className="empty">No ownership rules are configured; project defaults remain unchanged.</div>
                    ) : (
                      <div className="analytics-table-wrap">
                        <table className="analytics-table">
                          <thead>
                          ...
                          <tbody>
                            {ownershipDraft.rules.map((rule) => (
                              <tr key={rule.key}>
                                <td>
                                  <strong>{rule.name}</strong>
                                  <small>
                                    Priority {rule.priority} · {rule.key}
                                  </small>
                                </td>
                                <td>
                                  {[
                                    rule.category && `category ${rule.category}`,
                                    rule.repository && `repository ${rule.repository}`,
                                ...
                                <td>
                                  {[
                                    ...rule.owners.principalIds,
                                    ...rule.owners.roles,
                                    ...rule.owners.teamKeys.map((key) => `team:${key}`),
                                  ].join(", ") || "None"}
                                </td>
                                <td>
                                  {[
                                    ...rule.reviewers.principalIds,
                                    ...rule.reviewers.roles,
                                    ...rule.reviewers.teamKeys.map((key) => `team:${key}`),
                                  ].join(", ") || "None"}
                                </td>
                                <td>
                                  <button
                                    ...
                                    onClick={() =>
                                      setOwnershipDraft({
                                        ...ownershipDraft,
                                        rules: ownershipDraft.rules.filter((candidate) => candidate !== rule),
                                      })
                                    }
                                  >
                                    Remove
                  ...
                  <div className="member-form-actions">
                    <small>
                      Version {ownershipConfiguration?.version ?? 0}
                      {ownershipConfiguration?.updatedAt
                        ? ` · last saved ${new Date(ownershipConfiguration.updatedAt).toLocaleString()}`
                        : " · not saved yet"}
                    </small>
                    <button
                      className="primary"
            ...
            <>
              <div className="title-row">
                <div>
                  ...
                  <p>
                    Configure limited category/scope rules for risk, interruption, ownership, and protected review.
                    Bridge safety floors cannot be weakened.
                  </p>
                </div>
                <div className="member-form-actions">
                  <button
                  ...
                  <button
                    ...
                    disabled={
                      ...
                      !policyConfiguration ||
                      !policyDraft ||
                      JSON.stringify(policyConfiguration) === JSON.stringify(policyDraft)
                    }
              ...
              {!policyLoading && policyDraft ? (
                <div className="organization-stack">
                  <section className="organization-panel">
                    <div className="organization-panel-heading">
                      <div>
                        <h2>Pilot safety floors</h2>
                        <p>
                          These protected categories always block and retain their required human authority. Custom
                          rules may only add stricter requirements.
                        </p>
                      </div>
                    ...
                    <div className="policy-floor-summary" aria-label="Pilot safety floor summary">
                      <div>
                        <strong>{policyDraft.defaultRules.length}</strong>
                        <span>enforced categories</span>
                      </div>
                      <div>
                        <strong>
                          {new Set(policyDraft.defaultRules.flatMap((rule) => rule.requiredOwnerRoles)).size}
                        </strong>
                        <span>accountable owner roles</span>
                      </div>
                      <div>
                        <strong>
                          {new Set(policyDraft.defaultRules.flatMap((rule) => rule.requiredReviewerRoles)).size}
                        </strong>
                        <span>required reviewer roles</span>
                      </div>
                    </div>
                    <details className="inline-table-disclosure">
                      <summary>
                        View every enforced floor <span>{policyDraft.defaultRules.length}</span>
                      </summary>
                      <div className="analytics-table-wrap">
                        <table className="analytics-table">
                          <thead>
                          ...
                          <tbody>
                            {policyDraft.defaultRules.map((rule) => (
                              <tr key={rule.key}>
                                <td>
                                ...
                                <td>
                                  {rule.action.replaceAll("_", " ")} · {rule.minimumRisk}
                                </td>
                                <td>{rule.requiredOwnerRoles.join(", ") || "Policy owner"}</td>
                                <td>
                                  {[
                                    ...rule.requiredReviewerRoles,
                                    ...Object.entries(rule.reviewerQuorum ?? {}).map(
                                      ([role, count]) => `${role} ×${count}`,
                                    ),
                                  ].join(", ") || "No separate reviewer"}
                                </td>
                              </tr>
                            ))}
                          </tbody>
                  ...
                  <section className="organization-panel">
                    <div className="organization-panel-heading">
                      <div>
                        <h2>Project rules</h2>
                        <p>
                          Lower priority numbers win. Empty category/scope fields match broadly; equal-priority overlaps
                          are rejected.
                        </p>
                      </div>
                    ...
                    <details className="inline-create-disclosure">
                      ...
                      <form className="member-form-grid" onSubmit={addPolicyRule}>
                        ...
                        <label>
                          Rule name
                          <input name="policyName" placeholder="Block transfer quality changes" required />
                        </label>
                        <label>
                          Priority
                          <input name="policyPriority" type="number" min={1} max={1000} defaultValue={100} required />
                        </label>
                        <label>
                          Category
                          <input name="policyCategory" placeholder="quality" />
                        </label>
                        <label>
                          Repository
                          <input name="policyRepository" placeholder="payments-api" />
                        </label>
                        <label>
                          Component
                          <input name="policyComponent" placeholder="transfers" />
                        </label>
                        <label>
                          Branch
                          <input name="policyBranch" placeholder="main" />
                        </label>
                        <label>
                          Environment
                          <input name="policyEnvironment" placeholder="production" />
                        </label>
                        <label>
                          Work item
                          <input name="policyWorkItem" placeholder="PAY-142" />
                        </label>
                        <label>
                          Interruption action
                          <select name="policyAction" defaultValue="block">
                            <option value="assume_and_log">Assume and log</option>
                            <option value="ask_async">Ask asynchronously</option>
                            <option value="block">Block</option>
                            <option value="protected_approval">Protected approval</option>
                          </select>
                        </label>
                        <label>
                          Minimum risk
                          <select name="policyRisk" defaultValue="high">
                        ...
                        <label>
                          ...
                          <input name="policyOwnerRoles" placeholder="qa-lead, component-owner" />
                        </label>
                        <label>
                          Required reviewer roles
                          <input name="policyReviewerRoles" placeholder="architecture-reviewer" />
                        </label>
                        <label>
                          Reviewer quorum
                          <input name="policyReviewerQuorum" placeholder="security-reviewer=2" />
                        </label>
                        <div className="member-form-actions">
                          <small>
                            Use comma-separated normalized role names. Quorum uses role=count pairs and is valid only
                            for required reviewer roles on protected approval rules.
                          </small>
                          <button className="secondary" type="submit">
                            Add rule
                          </button>
                    ...
                    {policyDraft.rules.length === 0 ? (
                      <div className="empty">No custom rules are configured; Bridge defaults remain active.</div>
                    ) : (
                      <div className="analytics-table-wrap">
                        <table className="analytics-table">
                          <thead>
                            ...
                            {policyDraft.rules.map((rule) => (
                              <tr key={rule.key}>
                                ...
                                <td>
                                  {[
                                    rule.category && `category ${rule.category}`,
                                    ...Object.entries(rule.scope).map(([field, value]) => `${field} ${value}`),
                                  ]
                                    .filter(Boolean)
                                    .join(" · ") || "Project-wide"}
                                </td>
                                <td>
                                  {rule.action.replaceAll("_", " ")} · minimum {rule.minimumRisk}
                                </td>
                                <td>
                                  {[
                                    ...rule.requiredOwnerRoles.map((role) => `owner:${role}`),
                                    ...rule.requiredReviewerRoles.map((role) => `reviewer:${role}`),
                                    ...Object.entries(rule.reviewerQuorum ?? {}).map(
                                      ([role, count]) => `quorum:${role}=${count}`,
                                    ),
                                  ].join(", ") || "No added roles"}
                                </td>
                                <td>
                                  <button
                                    ...
                                    onClick={() =>
                                      setPolicyDraft({
                                        ...policyDraft,
                                        rules: policyDraft.rules.filter((candidate) => candidate !== rule),
                                      })
                                    }
                                  >
                                    Remove
                  ...
                  <div className="member-form-actions">
                    <small>
                      Version {policyConfiguration?.version ?? 0}
                      {policyConfiguration?.updatedAt
                        ? ` · last saved ${new Date(policyConfiguration.updatedAt).toLocaleString()}`
                        : " · no custom policy saved yet"}
                    </small>
                    <button
                      className="primary"
                      type="button"
            ...
            <>
              <div className="title-row">
                <div>
                  ...
                  <p>
                    Provision OIDC identities, suspend access, and assign organization or project roles with optimistic
                    version checks.
                  </p>
                </div>
                <button className="secondary" type="button" onClick={() => void loadOrganizationMembers()}>
                  Refresh
                </button>
              </div>
              ...
              {!organizationMembersLoading ? (
                <div className="organization-stack">
                  <details className="panel-disclosure">
                    <summary>
                      <span>
                        <strong>Add an OIDC member</strong>
                        <small>Provision a human identity and bounded project access.</small>
                      </span>
                      <em>Organization administrator</em>
                    </summary>
                    <form
                    ...
                    >
                      <p className="panel-form-intro">
                        The subject must match the configured identity provider exactly.
                      </p>
                      <div className="member-form-grid">
                        <label>
                          Display name
                        ...
                        <label>
                          ...
                          <input
                            ...
                            onChange={(event) => setNewMemberSubject(event.target.value)}
                            required
                        ...
                        <label>
                          ...
                          <input
                            ...
                            onChange={(event) => setNewMemberRoles(event.target.value)}
                            placeholder="organization-member, business-analyst"
                          />
                        </label>
                        <label>
                          Initial project
                          <select
                            ...
                            onChange={(event) => setNewMemberProjectId(event.target.value)}
                          >
                        ...
                        <label>
                          ...
                          <input
                            value={newMemberProjectRoles}
                            onChange={(event) => setNewMemberProjectRoles(event.target.value)}
                            disabled={!newMemberProjectId}
                          />
                        </label>
                        <label className="checkbox-label">
                          <input
                            type="checkbox"
                            checked={newMemberAllProjects}
                            onChange={(event) => setNewMemberAllProjects(event.target.checked)}
                          />
                          Access every organization project
                        </label>
                      </div>
                      <div className="member-form-actions">
                        <small>
                          Role names are normalized and duplicates are removed. Creating members never grants approval
                          to an agent identity.
                        </small>
                        <button
                          className="primary"
                          type="submit"
                          disabled={memberSubmitting || !newMemberName.trim() || !newMemberSubject.trim()}
                        >
                          Add member
                        </button>
                      </div>
                  ...
                  <section className="organization-panel">
                    <div className="organization-panel-heading">
                      <div>
                        <h2>Directory and access</h2>
                        <p>Disabled memberships fail authentication on the next request.</p>
                      </div>
                      <small>{organizationMembers.length} members</small>
                    </div>
                    {organizationMembers.length === 0 ? (
                      <div className="empty">No organization members are configured.</div>
                    ) : (
                        ...
                        <div className="member-list" aria-label="Organization members">
                          {organizationMembers.map((member) => (
                            <button
                              ...
                              type="button"
                              className={
                                selectedOrganizationMember?.id === member.id ? "member-row selected" : "member-row"
                              }
                              onClick={() => setSelectedMemberId(member.id)}
                            >
                              <span>
                                <strong>{member.displayName}</strong>
                                <small>{member.roles.join(" · ") || "No organization roles"}</small>
                              </span>
                        ...
                        {selectedOrganizationMember ? (
                          ...
                          >
                            <div className="member-editor-heading">
                              <div>
                                <h3>{selectedOrganizationMember.displayName}</h3>
                                <small>OIDC subject · {selectedOrganizationMember.oidcSubject}</small>
                              </div>
                            ...
                            <label>
                              ...
                              <select
                                value={memberStatus}
                                onChange={(event) =>
                                  setMemberStatus(event.target.value as OrganizationMember["status"])
                                }
                              >
                            ...
                            <label className="checkbox-label">
                              <input
                                type="checkbox"
                                checked={memberAllProjects}
                                onChange={(event) => setMemberAllProjects(event.target.checked)}
                              />
                              Access every organization project
                            </label>
                            <fieldset className="project-membership-editor">
                              ...
                              {organizationProjects.length === 0 ? (
                                <small>No projects are registered.</small>
                              ) : (
                                organizationProjects.map((project) => {
                                    ...
                                    <div key={project.id} className="project-membership-row">
                                      <label className="checkbox-label">
                                        <input
                                          ...
                                          checked={enabled}
                                          onChange={(event) =>
                                            setMemberProjectRoles((current) => ({
                                              ...current,
                                              [project.id]: event.target.checked
                                                ? (current[project.id] ?? "contributor")
                                                : undefined,
                                            }))
                                          }
                                        />
                            ...
                            <div className="member-form-actions">
                              ...
                              <button className="primary" type="submit" disabled={memberSubmitting}>
                                Save access
                              </button>
                            </div>
                          </form>
              ...
              <div className="notification-preference-panel" aria-label="Notification preferences">
                ...
                <label>
                  ...
                  <select
                    ...
                    onChange={(event) =>
                      void saveNotificationPreference(event.target.value as NotificationDeliveryPreference)
                    }
                  >
                  ...
                  {notifications.map((notification) => (
                    ...
                    >
                      ...
                      <span className="notification-copy">
                        ...
                        <small>
                          {new Date(notification.createdAt).toLocaleString()} · {notification.type.replaceAll("_", " ")}
                        </small>
                      </span>
            ...
            <>
              <div className="title-row">
                <div>
                  <h1>Audit events</h1>
                  <p>
                    Permission-restricted operational metadata. Record bodies, prompts, answers, credentials, and
                ...
                <div className="audit-actions">
                  <details className="action-menu">
                    ...
                    <div>
                      ...
                      <button type="button" disabled={auditExporting} onClick={() => void exportAudit("json")}>
                        Audit as JSON
                      </button>
                      <button type="button" disabled={auditExporting} onClick={() => void exportAudit("csv")}>
                        Audit as CSV
                      </button>
                    </div>
                  </details>
                  <button className="secondary" type="button" disabled={auditLoading} onClick={() => void loadAudit(0)}>
                    Refresh
                  </button>
                </div>
              </div>
              <details className="filter-disclosure">
                <summary>
                  Filter audit events <span>Optional</span>
                </summary>
                <div className="audit-filter-panel" aria-label="Audit filters">
                  ...
                  <label>
                    ...
                    <input
                      ...
                      onChange={(event) => updateAuditFilter("subjectType", event.target.value)}
                      placeholder="artifact_version"
                    />
                  </label>
                  <label>
                    Subject ID
                    <input
                      value={auditFilters.subjectId ?? ""}
                      onChange={(event) => updateAuditFilter("subjectId", event.target.value)}
                    />
                  </label>
                  <label>
                    Correlation ID
                    <input
                      ...
                      onChange={(event) => updateAuditFilter("correlationId", event.target.value)}
                    />
                  </label>
                  <label>
                    Created from
              ...
              {!auditLoading && auditPage && auditPage.items.length > 0 ? (
                <section className="analytics-panel audit-panel">
                  <div className="analytics-panel-heading">
                    <div>
                      <h2>Immutable event stream</h2>
                      <p>
                        {auditPage.totalMatching} matching events · showing {auditPage.offset + 1}–
                        {auditPage.offset + auditPage.items.length}
                      </p>
                    </div>
                        ...
                        {auditPage.items.map((event) => (
                          <tr key={event.id}>
                            ...
                            <td>
                              ...
                              event.afterVersion !== undefined ? (
                                <small>
                                  ...
                                  {event.beforeVersion === undefined && event.afterVersion === undefined
                                    ? ""
                                    : ` · v${event.beforeVersion ?? 0} → v${event.afterVersion ?? "?"}`}
                                </small>
                              ) : null}
                              {event.assignmentId ? (
                                <>
                                  ...
                                  <small>
                                    assignment
                                    {event.ownerRouteSource
                                      ? ` · owner via ${event.ownerRouteSource.replaceAll("_", " ")}`
                                      : ""}
                                    {event.reviewerRouteSource
                                      ? ` · reviewer via ${event.reviewerRouteSource.replaceAll("_", " ")}`
                                      : ""}
                                  </small>
                                </>
                              ) : null}
                            ...
                            <td>
                              ...
                              <small>
                                {event.actorType}
                                {event.source ? ` · ${event.source}` : " · legacy source unavailable"}
                              </small>
                            </td>
                            <td>
                              <strong>{event.subjectType}</strong>
                  ...
                  <div className="audit-pagination">
                    <button
                      className="secondary"
                      type="button"
                      disabled={auditLoading || auditPage.offset === 0}
                      onClick={() => void loadAudit(Math.max(0, auditPage.offset - auditPage.limit))}
                    >
                      Previous
                    </button>
                    <span>
                      Offset {auditPage.offset} of {auditPage.totalMatching}
                    </span>
                    <button
                      className="secondary"
                      type="button"
                      disabled={auditLoading || auditPage.nextOffset === undefined}
                      onClick={() => void loadAudit(auditPage.nextOffset ?? auditPage.offset)}
                    >
                      Next
                    </button>
                  </div>
            ...
            <>
              <div className="title-row">
                <div>
                  ...
                  <p>
                    Lifecycle counts and outcomes derived from governed records without exposing prompts, answers,
                    specifications, or hidden reasoning.
                  </p>
                </div>
                <button className="secondary" type="button" onClick={() => void loadAnalytics()}>
                  Refresh
                </button>
              </div>
              <details className="filter-disclosure">
                <summary>
                  Filter analytics cohort <span>Optional</span>
                </summary>
                <div className="filter-bar" aria-label="Analytics cohort filters">
                  ...
                  />
                  <button className="secondary" type="button" onClick={() => setAnalyticsFilters({})}>
                    Clear filters
                  </button>
                </div>
              </details>
                ...
                <div className="analytics-stack">
                  <div className="analytics-grid" aria-label="Pilot outcome summary">
                    ...
                    <article className="analytics-card">
                      ...
                      <span>
                        {formatPercent(analytics.outcomes.decisionAcceptanceRate)} of created questions accepted
                      </span>
                    </article>
                    <article className="analytics-card">
                  ...
                  <section className="analytics-panel">
                    <div className="analytics-panel-heading">
                      <div>
                        ...
                        <p>Counts follow runs selected by the cohort filters.</p>
                      </div>
                  ...
                  <section className="analytics-panel">
                    <div className="analytics-panel-heading">
                      <div>
                        <h2>Guardrails</h2>
                        <p>Signals that should stay bounded during the pilot.</p>
                      </div>
                    </div>
                    <div className="analytics-activity-grid">
                      <div>
                        <span>Questions per run</span>
                        <strong>{analytics.guardrails.questionsPerRun.toFixed(2)}</strong>
                      </div>
                      <div>
                        <span>Blocking questions</span>
                        <strong>{analytics.guardrails.blockingQuestions}</strong>
                      </div>
                      <div>
                        <span>Unrouted blocking questions</span>
                        <strong>{analytics.guardrails.unroutedBlockingQuestions}</strong>
                      </div>
                      <div>
                        <span>Context items per retrieval</span>
                        <strong>{analytics.guardrails.contextItemsPerRetrieval.toFixed(2)}</strong>
                      </div>
                      <div>
                        <span>First-assignment routing</span>
                        <strong>{formatPercent(analytics.outcomes.firstAssignmentRoutingRate)}</strong>
                      </div>
                      <div>
                        <span>Assumption resolution</span>
                        <strong>{formatPercent(analytics.outcomes.assumptionResolutionRate)}</strong>
                      </div>
                    </div>
                  </section>
                  ...
                  <section className="analytics-panel">
                    <div className="analytics-panel-heading">
                      <div>
                        <h2>Agent client breakdown</h2>
                        <p>Controlled client names only; task and user content are excluded.</p>
                      </div>
                    </div>
                    {analytics.byClient.length === 0 ? (
                      <div className="empty">No runs match this cohort.</div>
                    ) : (
                      <div className="analytics-table-wrap">
                        <table className="analytics-table">
                          <thead>
                            <tr>
                              ...
                              <th>Context</th>
                              <th>Questions</th>
                              <th>Reused questions</th>
                              <th>Accepted decisions</th>
                              <th>Decision reuse</th>
                            </tr>
                          </thead>
                          <tbody>
                  ...
                  <section className="analytics-panel privacy-panel">
                    <div className="analytics-panel-heading">
                      <div>
                        ...
                        <p>This view calculates metadata in place and does not create a second content store.</p>
                      </div>
                    </div>
                    <div className="privacy-columns">
                      <div>
                        <h3>Derived from</h3>
                        <ul>
                          {analytics.privacy.derivedFrom.map((item) => (
                            <li key={item}>{item}</li>
                          ))}
                        </ul>
                      </div>
                      <div>
                        ...
                        <ul>
                          {analytics.privacy.excluded.map((item) => (
                            <li key={item}>{item}</li>
                          ))}
                        </ul>
                      </div>
            ...
            <>
              <div className="title-row">
                <div>
                  <h1>Accepted project decisions</h1>
                  <p>Only human-accepted answers appear here as authoritative context.</p>
                </div>
                <button className="secondary" type="button" onClick={() => void loadReferenceData()}>
                  Refresh
                </button>
              </div>
              <details className="filter-disclosure">
                <summary>
                  Filter decisions <span>{hasDecisionFilters ? "Filters active" : "Optional"}</span>
                </summary>
                    ...
                    {decisions.map((decision) => (
                      <button
                        ...
                        className={
                          decision.id === selectedDecision?.id
                            ? "question-row record-row selected"
                      ...
                      >
                        ...
                        <span className="record-row-copy">
                          ...
                          <small>
                            {decision.category} · {decision.scope.component ?? "project"}
                            {decisionConflicts.some(
                              (conflict) => conflict.left.id === decision.id || conflict.right.id === decision.id,
                            )
                              ? " · potential conflict"
                              : ""}
                          </small>
                      ...
                      <header className="record-detail-header">
                        <div className="record-title-block">
                          <span className="record-eyebrow">
                            Project decision · {selectedDecision.category.replaceAll("_", " ")}
                          </span>
                          ...
                          <div className="record-chip-row" aria-label="Decision scope">
                            {Object.entries(selectedDecision.scope).length > 0 ? (
                              Object.entries(selectedDecision.scope).map(([key, value]) => (
                                <span className="record-chip" key={key}>
                                  {key}: {value}
                                </span>
                              ))
                            ) : (
                      ...
                      <div className="record-detail-body">
                        <section className="record-section record-summary-section">
                          <div className="record-section-heading">
                            <div>
                              <span>Why this was accepted</span>
                        ...
                        <section className="record-section">
                          ...
                          <dl className="record-meta-grid">
                            <div>
                              <dt>Accepted by</dt>
                              <dd title={selectedDecision.ownerId}>
                                {displayIdentityName(selectedDecision.ownerId, principals)}
                              </dd>
                            </div>
                            <div>
                              <dt>Accepted at</dt>
                              <dd>{new Date(selectedDecision.createdAt).toLocaleString()}</dd>
                            </div>
                            <div>
                              <dt>Review due</dt>
                              <dd>{new Date(selectedDecision.reviewAt).toLocaleDateString()}</dd>
                            </div>
                            <div>
                              <dt>Decision version</dt>
                              <dd>Version {selectedDecision.version}</dd>
                            </div>
                          </dl>
                        </section>
                        {selectedDecisionConflicts.length > 0 ? (
                          <section className="record-section">
                            ...
                            <p className="muted-copy">
                              A human owner must inspect the scope and rationale before changing either decision.
                            </p>
                                ...
                                return (
                                  <article className="conflict-card" key={conflict.id}>
                                    <div>
                                      ...
                                      <span>
                                        {conflict.scopeRelation.replaceAll("_", " ")} scope ·{" "}
                                        {conflict.signals.join(" · ")}
                                      </span>
                                    </div>
                                    <p>{other.answer}</p>
                                    <button
                          ...
                          <section className="record-section">
                            ...
                            <div className="record-evidence">
                              ...
                              <dl className="record-meta-grid">
                                <div>
                                  <dt>Changed by</dt>
                                  <dd title={selectedDecision.lifecycleChangedById}>
                                    {displayIdentityName(
                                      selectedDecision.lifecycleChangedById ?? "Bridge worker",
                                      principals,
                                    )}
                                  </dd>
                                </div>
                                <div>
                                  <dt>Changed at</dt>
                                  <dd>{new Date(selectedDecision.lifecycleChangedAt).toLocaleString()}</dd>
                                </div>
                                {selectedDecision.replacementDecisionId ? (
                                  <div>
                                    <dt>Replacement</dt>
                                    <dd title={selectedDecision.replacementDecisionId}>Linked decision</dd>
                                  </div>
                                ) : null}
                              </dl>
                            </div>
                        ...
                        <section className="record-section">
                          <div className="record-action-panel">
                            <div>
                              ...
                              <p>
                                Preview direct and transitive records that may need review. Analysis is read-only and
                                does not alter human authority.
                              </p>
                            </div>
                            <button
                              className="secondary"
                              type="button"
                              disabled={decisionImpactLoading}
                              onClick={() => void loadDecisionImpact()}
                            >
                          ...
                          {decisionLifecycleImpact ? (
                            <div className="impact-analysis" aria-live="polite">
                              <p className="impact">
                                {decisionLifecycleImpact.artifactIds.length} specification(s),{" "}
                                {decisionLifecycleImpact.assumptionIds.length} assumption(s),{" "}
                                {decisionLifecycleImpact.questionIds.length} question(s),{" "}
                                {decisionLifecycleImpact.runIds.length} agent run(s), and{" "}
                                {decisionLifecycleImpact.workItems.length} work item(s).
                              </p>
                              <div className="spec-meta">
                                ...
                                {decisionLifecycleImpact.truncated ? (
                                  <span>Bound reached; narrow or rerun with API limits</span>
                                ) : (
                                  <span>Complete within configured bounds</span>
                                )}
                              </div>
                              <div className="impact-paths">
                                {decisionLifecycleImpact.nodes
                                  .filter((node) => node.depth > 0)
                                  .slice(0, 8)
                                  .map((node) => (
                                    <div key={node.id}>
                                      <strong>
                                        Level {node.depth} · {node.type.replaceAll("_", " ")}
                                      </strong>
                        ...
                        {selectedDecision.status === "active" ? (
                          <details className="detail-disclosure lifecycle-disclosure">
                            <summary>
                              ...
                              <small>Revoke, expire, or supersede</small>
                            </summary>
                            <div className="response-form lifecycle-form">
                              <p>
                                These actions remove this decision from active authoritative context. A human rationale
                                is always required.
                              </p>
                              ...
                              <select
                                ...
                                onChange={(event) =>
                                  setDecisionLifecycleStatus(event.target.value as typeof decisionLifecycleStatus)
                                }
                              >
                        ...
                        <footer className="record-footer">
                          <span>
                            {selectedDecision.questionId
                              ? "Trace this decision back to the human-reviewed question."
                              : "Created from a human-confirmed assumption."}
                          </span>
                          {selectedDecision.questionId ? (
                            <button
                              className="secondary"
            ...
            <>
              <div className="title-row">
                <div>
                  <h1>Visible project assumptions</h1>
                  <p>Assumptions are temporary premises with explicit risk, expiry, and reversal cost.</p>
                </div>
                <label>
                  Show
                  <select
                    value={assumptionStatusFilter}
                    onChange={(event) => setAssumptionStatusFilter(event.target.value as Assumption["status"] | "all")}
                  >
              ...
              {!referenceDataLoading && assumptions.length === 0 ? (
                <div className="empty">No assumptions have been recorded for this project.</div>
              ) : null}
              {!referenceDataLoading && assumptions.length > 0 && visibleAssumptions.length === 0 ? (
                <div className="empty">No assumptions match this status.</div>
              ) : null}
                  ...
                  {selectedAssumption ? (
                    <article className="question-detail record-detail">
                      <header className="record-detail-header">
                        <div className="record-title-block">
                          <span className="record-eyebrow">
                            Project assumption · version {selectedAssumption.version}
                          </span>
                          ...
                          <div className="record-chip-row" aria-label="Assumption attributes">
                            ...
                            <span className="record-chip">
                              {selectedAssumption.reversible ? "Reversible" : "Not reversible"}
                            </span>
                          </div>
                        </div>
                        <span className={`status status-${selectedAssumption.status}`}>
                          {selectedAssumption.status}
                        </span>
                      </header>
                      <div className="record-detail-body">
                        <section className="record-section record-summary-section">
                          <div className="record-section-heading">
                            <div>
                              <span>Temporary premise</span>
                              <h3>Why the team is using this assumption</h3>
                            </div>
                          </div>
                        ...
                        <section className="record-section">
                          <div className="record-section-heading">
                            <div>
                              ...
                              <h3>Expiry and reversibility</h3>
                            </div>
                          </div>
                          <div className="assumption-cost">
                            <span>Reversal cost</span>
                            <strong>{selectedAssumption.reversalCost}</strong>
                          </div>
                          <dl className="record-meta-grid">
                            <div>
                              <dt>Risk</dt>
                              <dd>{selectedAssumption.risk}</dd>
                            </div>
                            <div>
                              <dt>Confidence</dt>
                              <dd>{selectedAssumption.confidence}</dd>
                            </div>
                            <div>
                              <dt>Expires</dt>
                              <dd>{new Date(selectedAssumption.expiresAt).toLocaleString()}</dd>
                            </div>
                            <div>
                              <dt>Created by</dt>
                              <dd title={selectedAssumption.createdById}>
                                {displayIdentityName(selectedAssumption.createdById, principals)}
                              </dd>
                            </div>
                            <div>
                              <dt>Scope</dt>
                              <dd>
                                {Object.entries(selectedAssumption.scope)
                        ...
                        {selectedAssumption.sourceLinks.length > 0 ? (
                          <section className="record-section">
                            <div className="record-section-heading">
                              <div>
                                <span>Evidence</span>
                                <h3>Directly linked work</h3>
                              </div>
                            </div>
                            <div className="record-link-list">
                              {selectedAssumption.sourceLinks.map((link) => (
                          ...
                          <section className="record-section">
                            ...
                            <div className="record-evidence">
                              ...
                              <dl className="record-meta-grid">
                                <div>
                                  <dt>Resolved by</dt>
                                  <dd title={selectedAssumption.resolvedById}>
                                    {displayIdentityName(
                                      selectedAssumption.resolvedById ?? "Bridge worker",
                                      principals,
                                    )}
                                  </dd>
                                </div>
                                {selectedAssumption.confirmedDecisionId ? (
                                  <div>
                                    <dt>Authoritative decision</dt>
                                    <dd>Linked and active</dd>
                                  </div>
                                ) : null}
                              </dl>
                            </div>
                        ...
                        {canResolveSelectedAssumption && selectedAssumption.status === "active" ? (
                          <details className="detail-disclosure assumption-resolution-disclosure">
                            <summary>
                              <span>Resolve this assumption</span>
                              <small>Human action</small>
                            </summary>
                            <div className="response-form assumption-resolution-form">
                              <p className="muted-copy">
                                Confirm, reject, or expire this temporary premise. Only confirmation can create an
                              ...
                              {assumptionResolutionStatus === "confirmed" ? (
                                <label>
                                  <input
                                    type="checkbox"
                                    checked={assumptionCreateDecision}
                                    onChange={(event) => setAssumptionCreateDecision(event.target.checked)}
                                  />{" "}
            ...
            <>
              <div className="title-row">
                <div>
                  <h1>Agent runs and durable handoffs</h1>
                  <p>Runs link questions, assumptions, context snapshots, specifications, and completion outcomes.</p>
                </div>
                <button className="secondary" type="button" onClick={() => void loadReferenceData()}>
                  Refresh
                </button>
              ...
              {!referenceDataLoading && runs.length === 0 ? (
                <div className="empty">No agent runs have been registered for this project.</div>
              ) : null}
              {!referenceDataLoading && runs.length > 0 ? (
                <div className="decision-layout">
                  <div className="question-list" aria-label="Agent runs">
                    {runs.map((run) => (
                      <button
                        ...
                        key={run.id}
                        className={
                          run.id === selectedRun?.id ? "question-row record-row selected" : "question-row record-row"
                        }
                        onClick={() => setSelectedRunId(run.id)}
                      >
                        <span className="document-mark" aria-hidden="true">
                          <BridgeIcon name="runs" size={16} />
                        </span>
                        <span className="record-row-copy">
                          <strong>{run.taskSummary}</strong>
                          <small>
                            {run.client.replaceAll("_", " ")} · {run.capability.replaceAll("_", " ")} ·{" "}
                            {run.continuationMode} continuation · version {run.version}
                          </small>
                        </span>
                    ...
                    <article className="question-detail record-detail">
                      ...
                      <div className="record-detail-body">
                        <section className={`record-state record-state-${selectedRun.status}`}>
                          <span className="record-state-icon">
                            <BridgeIcon
                              name={selectedRun.status === "waiting_for_human" ? "questions" : "runs"}
                              size={20}
                            />
                          </span>
                          <div>
                            ...
                            <p>
                              {selectedRun.status === "waiting_for_human"
                                ...
                                : selectedRun.status === "running"
                                  ...
                                  : (selectedRun.summary ??
                                    "The durable run record preserves this outcome for later handoff.")}
                            </p>
                          </div>
                        </section>
                        <section className="record-section">
                          ...
                          <dl className="record-meta-grid">
                            <div>
                              <dt>Agent</dt>
                              <dd title={selectedRun.agentId}>
                                {displayIdentityName(selectedRun.agentId, principals)}
                              </dd>
                            </div>
                            <div>
                              <dt>Started</dt>
                            ...
                            <div>
                              <dt>Scope</dt>
                              <dd>
                                {Object.entries(selectedRun.scope)
                                  .map(([key, value]) => `${key}: ${value}`)
                                  .join(" · ") || "Project-wide"}
                              </dd>
                            </div>
                            {selectedRun.endedAt ? (
                              <div>
                                <dt>Ended</dt>
                                <dd>{new Date(selectedRun.endedAt).toLocaleString()}</dd>
                              </div>
                            ) : null}
                            {selectedRun.continuesRunId ? (
                              <div>
                                <dt>Continues run</dt>
                                <dd title={selectedRun.continuesRunId}>Linked predecessor</dd>
                              </div>
                            ) : null}
                          </dl>
                        </section>
                        <section className="record-section">
                          <div className="record-section-heading">
                            ...
                            <small>Select a question to review it</small>
                          </div>
                          <div className="record-stat-grid">
                            <div className="record-stat">
                              <strong>{selectedRun.contextSnapshotIds.length}</strong>
                            ...
                            {selectedRun.questionIds[0] ? (
                              ...
                              >
                                <strong>{selectedRun.questionIds.length}</strong>
                                <span>Questions</span>
                                <small>Open first question →</small>
                              </button>
                            ) : (
                              <div className="record-stat">
                                <strong>0</strong>
                            ...
                            <div className="record-stat">
                              <strong>{selectedRun.assumptionIds.length}</strong>
                              <span>Assumptions</span>
                            </div>
                            <div className="record-stat">
                              <strong>{selectedRun.artifactVersionIds.length}</strong>
                              <span>Specification versions</span>
                            </div>
                          </div>
                        </section>
                        {selectedRun.summary ? (
                        ...
                        {selectedRun.resultLinks.length > 0 ? (
                          <section className="record-section">
                            ...
                            <div className="record-link-list">
                              {selectedRun.resultLinks.map((link) => (
                                <a key={link} href={link} target="_blank" rel="noreferrer">
                                  {link}
                                </a>
                              ))}
            ...
            <>
                ...
                <div>
                  ...
                  <p>
                    {view === "inbox"
                      ...
                      : "Shared questions remain visible to the whole project team; use My Inbox for questions routed to you."}
                  </p>
                </div>
                <button
                  className="secondary"
                  type="button"
                  onClick={() =>
                    void Promise.all([loadProjects(), loadQuestions(), loadArtifacts(), loadNotifications()])
                  }
                >
                  Refresh
                </button>
              ...
              {view === "inbox" ? (
                <details className="filter-disclosure">
                  <summary>
                    Filter inbox <span>{hasInboxFilters ? "Filters active" : "Optional"}</span>
                  </summary>
                  <div className="filter-bar" aria-label="Inbox filters">
                    ...
                    >
                      <option value="">All categories</option>
                      {inboxFilterOptions.categories.map((category) => (
                        <option key={category} value={category}>
                          {category}
                        </option>
                      ))}
                    ...
                    >
                      ...
                      {inboxFilterOptions.roles.map((role) => (
                    ...
                    {hasInboxFilters ? (
                      <button className="secondary" type="button" onClick={() => setInboxFilters({})}>
                        Clear filters
                      </button>
                    ) : null}
                  </div>
              ...
              {view === "inbox" && questionDigests.length > 0 ? (
                <details className="digest-disclosure">
                  <summary>
                    Low-risk decision digests{" "}
                    <span>
                      {questionDigests.length} related {questionDigests.length === 1 ? "group" : "groups"}
                    </span>
                  </summary>
                    ...
                    {questionDigests.map((digest) => (
                      <article className="digest-card" key={digest.id}>
                        <div className="digest-heading">
                          <div>
                            <strong>{digest.category}</strong>
                            <small>
                              {Object.entries(digest.scope)
                        ...
                        <p>
                          Grouped because these items are low risk, non-blocking, and share the same category and scope.
                          Each still needs an individual human decision.
                        </p>
                        <div className="digest-questions">
                          {digest.questions.map((question) => (
                            ...
                            >
                              {question.title}
                              {question.dueAt ? ` · ${new Date(question.dueAt).toLocaleDateString()}` : ""}
                            </button>
                          ))}
                        </div>
                        {digest.remainingQuestionCount > 0 ? (
                          <small>+ {digest.remainingQuestionCount} more in this digest</small>
                        ) : null}
                      </article>
                    ))}
                  </div>
              ...
              {!questionsLoading && visibleQuestions.length === 0 ? (
                <div className="empty empty-state">
                  <span className="empty-icon">
                    <BridgeIcon name={view === "inbox" ? "sparkle" : "questions"} size={23} />
                  </span>
                  <h2>
                    {view === "inbox"
                      ? hasInboxFilters
                  ...
                  <p>
                    {view === "inbox"
                      ...
                      : "Questions raised by agents and teammates will appear here with owner, risk, and approval context."}
                  </p>
                  {view === "inbox" ? (
                    <button className="secondary" type="button" onClick={() => navigateTo("questions")}>
                    ...
                    {visibleQuestions.map((question) => (
                      ...
                      >
                        ...
                        <span>
                          <strong>{question.title}</strong>
                          <small>
                            {question.category} · {question.scope.component ?? "project"}
                            {question.dueAt
                              ? ` · ${question.dueStatus.replaceAll("_", " ")} ${new Date(question.dueAt).toLocaleDateString()}`
                              : ""}
                            {question.blockingEscalatedAt ? " · escalated" : ""}
                          </small>
                        </span>
                  ...
                  {selectedQuestion ? (
                    <article className="question-detail record-detail question-workspace-detail">
                      <header className="record-detail-header">
                        <div className="record-title-block">
                          <span className="record-eyebrow">
                            Governed question · {selectedQuestion.category.replaceAll("_", " ")}
                          </span>
                      ...
                      <div className="record-detail-body">
                        <section className="record-section question-context-section">
                          ...
                          <div className="question-impact-panel">
                            <span>Why it matters</span>
                            <p>{selectedQuestion.whyItMatters}</p>
                          </div>
                          <dl className="record-meta-grid question-authority-grid">
                            <div>
                              ...
                              <dd>
                                {[
                                  ...selectedQuestion.ownerIds.map((id) => displayIdentityName(id, principals)),
                                  ...selectedQuestion.ownerRoles,
                                ].join(" · ") || "Project administrator"}
                              </dd>
                            </div>
                            <div>
                              <dt>Review lane</dt>
                              <dd>
                                {[
                                  ...selectedQuestion.reviewerIds.map((id) => displayIdentityName(id, principals)),
                                  ...selectedQuestion.reviewerRoles,
                                ].join(" · ") || "No separate reviewer"}
                              </dd>
                            </div>
                            <div>
                              <dt>Policy</dt>
                              <dd title={selectedQuestion.policyRuleKey}>
                                {selectedQuestion.policyAction.replaceAll("_", " ")} · version{" "}
                                {selectedQuestion.policyVersion}
                              </dd>
                            </div>
                            <div>
                              <dt>Scope</dt>
                              <dd>
                                {Object.entries(selectedQuestion.scope)
                                  .map(([field, value]) => `${field}: ${value}`)
                                  .join(" · ") || "Project-wide"}
                              </dd>
                            </div>
                            {selectedQuestion.dueAt ? (
                              <div>
                                <dt>Due</dt>
                                <dd>
                                  {new Date(selectedQuestion.dueAt).toLocaleString()} ·{" "}
                                  {selectedQuestion.dueStatus.replaceAll("_", " ")}
                                </dd>
                              </div>
                            ) : null}
                            {selectedQuestion.blockingEscalatedAt ? (
                              <div>
                                <dt>Escalated</dt>
                                <dd>{new Date(selectedQuestion.blockingEscalatedAt).toLocaleString()}</dd>
                              </div>
                            ) : null}
                          </dl>
                          {selectedQuestion.approvalStatus.requirements.length > 0 ? (
                            <div className="question-approval-progress">
                              ...
                              <strong>
                                {selectedQuestion.approvalStatus.requirements
                                  .map(
                                    (requirement) =>
                                      `${requirement.role} ${requirement.approvedCount}/${requirement.requiredCount} ${requirement.status}`,
                                  )
                                  .join(" · ")}
                              </strong>
                            </div>
                          ...
                          {view === "inbox" && selectedQuestion.inboxReasons?.length ? (
                            <div className="inbox-reason">
                              <strong>Inbox routing:</strong>{" "}
                              {selectedQuestion.inboxReasons.map((reason) => reason.replaceAll("_", " ")).join(" · ")}
                              {selectedQuestion.canAccept === false
                                ? " · security review is also required before acceptance"
                                : ""}
                            </div>
                          ) : null}
                        </section>
                          ...
                          <div className="detail-tool-grid">
                            <details className="nested-tool">
                              ...
                              <div className="audience-controls">
                                <label htmlFor="question-audience-role">
                                  Audience role
                                  <select
                                ...
                                <div className="audience-actions">
                                  <button
                                    className="secondary"
                                    type="button"
                                    disabled={questionAudienceLoading}
                                    onClick={() => void loadQuestionAudienceView("explain")}
                                  >
                                  ...
                                  <button
                                    ...
                                    type="button"
                                    disabled={questionAudienceLoading}
                                    onClick={() => void loadQuestionAudienceView("rewrite")}
                                  >
                                    Rewrite for role
                                  </button>
                                </div>
                              ...
                              {questionAudienceView ? (
                                <div className="audience-view" aria-live="polite">
                                  <small>
                                    Derived {questionAudienceView.mode} for {questionAudienceView.role}. The recorded
                                    question and human approval authority are unchanged.
                                  </small>
                                  ...
                                  <div className="audience-focus">
                                    ...
                                    {questionAudienceView.presentation.focusAreas.join(" · ")}
                                  </div>
                                  <div className="impact">
                                    <strong>Why it matters:</strong> {questionAudienceView.presentation.whyItMatters}
                                  </div>
                                  <p>
                                    <strong>Review prompt:</strong> {questionAudienceView.presentation.reviewPrompt}
                                  </p>
                                </div>
                              ) : null}
                            </details>
                            ...
                            <details className="nested-tool">
                              <summary>
                                Provenance{" "}
                                <span className="section-count">{selectedQuestion.runId ? "linked" : "direct"}</span>
                              </summary>
                              ...
                              <details className="nested-tool">
                                ...
                                {selectedQuestion.canRequestClarification ? (
                                  <div className="response-form">
                                    ...
                                    <p className="muted-copy">
                                      This moves an open question into discussion and records why the owner needs more
                                      information.
                                    </p>
                                    ...
                                    <button
                                      className="secondary"
                                      type="button"
                                      disabled={submitting || clarificationReason.trim().length < 10}
                                      onClick={() => void requestQuestionClarification()}
                                    >
                                ...
                                {selectedQuestion.canReopen ? (
                                  <div className="response-form">
                                    ...
                                    <p className="muted-copy">
                                      Only cancelled or expired questions can be reopened. An accepted decision remains
                                      authoritative until its own lifecycle changes.
                                    </p>
                                    ...
                                    <button
                                      className="secondary"
                                      type="button"
                                      disabled={submitting || reopenReason.trim().length < 10}
                                      onClick={() => void reopenQuestion()}
                                    >
                            ...
                            <details className="nested-tool">
                              <summary>
                                Assignment routing{" "}
                                <span className="section-count">{selectedQuestion.assignmentHistory.length}</span>
                              </summary>
                              <div className="owner-routing">
                                <strong>Current route:</strong> owner via{" "}
                                {selectedQuestion.routing.ownerSource.replaceAll("_", " ")}
                                {selectedQuestion.routing.ownerRuleKey
                                  ? ` (${selectedQuestion.routing.ownerRuleKey})`
                                  : ""}{" "}
                                · reviewer via {selectedQuestion.routing.reviewerSource.replaceAll("_", " ")}
                                {selectedQuestion.routing.reviewerRuleKey
                                  ? ` (${selectedQuestion.routing.reviewerRuleKey})`
                                  : ""}{" "}
                                · ownership v{selectedQuestion.routing.ownershipVersion} · policy v
                                {selectedQuestion.routing.policyVersion}
                              </div>
                              <div className="response-list">
                                {selectedQuestion.assignmentHistory.map((assignment) => (
                                  <article className="response-card" key={assignment.id}>
                                    <div className="response-heading">
                                      <strong>{assignment.kind === "initial" ? "Initial route" : "Reassigned"}</strong>
                                      <small>{new Date(assignment.createdAt).toLocaleString()}</small>
                                    </div>
                                    <small>
                                      By {assignment.changedById} · question v{assignment.questionVersion}
                                    </small>
                                    <div className="response-rationale">
                                      <strong>Owners:</strong>{" "}
                                      {[...assignment.ownerIds, ...assignment.ownerRoles].join(", ") ||
                                        "Administrator fallback"}
                                    </div>
                                    <div className="response-rationale">
                                      <strong>Reviewers:</strong>{" "}
                                      {[...assignment.reviewerIds, ...assignment.reviewerRoles].join(", ") || "None"}
                                    </div>
                                    {assignment.reason ? (
                                      <div className="response-rationale">
                              ...
                              {selectedQuestion.canReassign ? (
                                <form className="member-form-grid" onSubmit={(event) => void reassignQuestion(event)}>
                                  <label>
                                    Owner member IDs
                                    <input
                                      name="assignmentOwnerIds"
                                      defaultValue={selectedQuestion.ownerIds.join(", ")}
                                      placeholder="usr_architect"
                                    />
                                  </label>
                                  <label>
                                    Owner roles
                                    <input
                                      name="assignmentOwnerRoles"
                                      defaultValue={selectedQuestion.ownerRoles.join(", ")}
                                      placeholder="qa-lead"
                                    />
                                  </label>
                                  <label>
                                    Reviewer member IDs
                                    <input
                                      name="assignmentReviewerIds"
                                      defaultValue={selectedQuestion.reviewerIds.join(", ")}
                                      placeholder="usr_qa_lead"
                                    />
                                  </label>
                                  <label>
                                    Reviewer roles
                                    <input
                                      name="assignmentReviewerRoles"
                                      defaultValue={selectedQuestion.reviewerRoles.join(", ")}
                                  ...
                                  <label>
                                    ...
                                    <textarea
                                      ...
                                      minLength={10}
                                      required
                                      placeholder="Explain why accountable ownership or review changed."
                                    />
                                  </label>
                                  <div className="member-form-actions">
                                    <small>
                                      Active humans: {principals.map((principal) => principal.id).join(", ") || "none"}.
                                      Required policy roles are retained automatically.
                                    </small>
                        ...
                        <section className="record-section question-options-section">
                          <div className="record-section-heading">
                            ...
                            <small>The recommendation remains advisory</small>
                          </div>
                          <div className="options">
                            {selectedQuestion.options.map((option) => (
                              ...
                              >
                                <strong>
                                  {option.label}
                                  {option.key === selectedQuestion.recommendationKey ? (
                                    <em>Agent recommendation</em>
                                  ) : null}
                                </strong>
                        ...
                        <details className="detail-disclosure">
                          <summary>
                            Team discussion <span className="section-count">{selectedQuestion.responses.length}</span>
                          </summary>
                          {selectedQuestion.responses.length === 0 ? (
                            <p className="muted-copy">
                              No responses yet. Share your perspective so the decision owner can compare the trade-offs.
                            </p>
                          ) : (
                            <div className="response-list">
                                ...
                                return (
                                  <article className="response-card" key={response.id}>
                                    ...
                                    {responseEditing ? (
                                      <div className="response-form">
                                        <label>
                                          ...
                                          <select
                                            ...
                                            onChange={(event) => setEditResponseOption(event.target.value || undefined)}
                                          >
                                        ...
                                        <label>
                                          Answer
                                          <textarea
                                            value={editResponseAnswer}
                                            onChange={(event) => setEditResponseAnswer(event.target.value)}
                                          />
                                        </label>
                                        <label>
                                          Why
                                          <textarea
                                            value={editResponseRationale}
                                            onChange={(event) => setEditResponseRationale(event.target.value)}
                                          />
                                        </label>
                                        <label>
                                          Mention human IDs (optional)
                                          <input
                                            value={editResponseMentionIds}
                                            onChange={(event) => setEditResponseMentionIds(event.target.value)}
                                        ...
                                        <div className="member-form-actions">
                                          ...
                                          <button
                                            className="text-button"
                                            type="button"
                                            onClick={() => setEditingResponseId(undefined)}
                                          >
                                            Cancel
                                          </button>
                                        </div>
                                      ...
                                      <>
                                        {responseOptionLabel ? (
                                          <span className="response-option">Selected: {responseOptionLabel}</span>
                                        ) : null}
                                        <p>{response.answer}</p>
                                        <div className="response-rationale">
                                          <strong>Rationale:</strong> {response.rationale}
                                        </div>
                                        {response.mentionedPrincipalIds && response.mentionedPrincipalIds.length > 0 ? (
                                          <small>Mentioned: {response.mentionedPrincipalIds.join(", ")}</small>
                                        ) : null}
                                        {response.revisionHistory && response.revisionHistory.length > 0 ? (
                                          <details className="nested-disclosure">
                                            <summary>
                                              Edit history{" "}
                                              <span className="section-count">{response.revisionHistory.length}</span>
                                            </summary>
                                            {response.revisionHistory.map((revision) => (
                                              <div className="history-item" key={revision.id}>
                                                <small>
                                                  {new Date(revision.editedAt).toLocaleString()} · edited by{" "}
                                                  {revision.editedById}
                                                </small>
                                                <p>{revision.answer}</p>
                                                <div className="response-rationale">
                                                  <strong>Previous rationale:</strong> {revision.rationale}
                                                </div>
                                              </div>
                                            ))}
                                          </details>
                                        ) : null}
                                        {selectedQuestion.editableResponseIds.includes(response.id) ? (
                                          <button
                                            className="text-button"
                                            type="button"
                                            onClick={() => beginResponseEdit(response)}
                                          >
                                            Edit response
                                          </button>
                                        ) : null}
                          ...
                          {selectedQuestion.status !== "accepted" ? (
                            <div className="response-form">
                              ...
                              <button
                                ...
                                type="button"
                                disabled={
                                  submitting || responseAnswer.trim().length < 2 || responseRationale.trim().length < 2
                                }
                        ...
                        <details className="detail-disclosure">
                          <summary>
                            Clarifications <span className="section-count">{selectedQuestion.comments.length}</span>
                          </summary>
                          {selectedQuestion.comments.length === 0 ? (
                            <p className="muted-copy">
                              No clarification thread yet. Ask a focused follow-up so the team can resolve missing
                              context without reopening the agent session.
                            </p>
                          ) : (
                            <div className="comment-list">
                              {selectedQuestion.comments.map((comment) => {
                                const parent = comment.parentCommentId
                                  ? selectedQuestion.comments.find(
                                ...
                                return (
                                  <article
                                    className={comment.parentCommentId ? "comment-card comment-reply" : "comment-card"}
                                    key={comment.id}
                                  >
                                    <div className="response-heading">
                                    ...
                                    {commentEditing ? (
                                      <div className="response-form">
                                        ...
                                        <label>
                                          Mention human IDs (optional)
                                          <input
                                            value={editCommentMentionIds}
                                            onChange={(event) => setEditCommentMentionIds(event.target.value)}
                                            placeholder="usr_qa_lead, usr_architect"
                                          />
                                        </label>
                                        <div className="member-form-actions">
                                      ...
                                      <>
                                        <p>{comment.body}</p>
                                        {comment.mentionedPrincipalIds && comment.mentionedPrincipalIds.length > 0 ? (
                                          <small>Mentioned: {comment.mentionedPrincipalIds.join(", ")}</small>
                                        ) : null}
                                        {comment.revisionHistory && comment.revisionHistory.length > 0 ? (
                                          <details className="nested-disclosure">
                                            <summary>
                                              Edit history{" "}
                                              <span className="section-count">{comment.revisionHistory.length}</span>
                                            </summary>
                                        ...
                                        {selectedQuestion.editableCommentIds.includes(comment.id) ? (
                                          <button
                                            className="text-button"
                                            type="button"
                            ...
                            <div className="response-form">
                              {replyToCommentId ? (
                                <div className="replying-to">
                                  Replying to{" "}
                                  {selectedQuestion.comments.find((comment) => comment.id === replyToCommentId)
                                    ?.authorId ?? "comment"}
                                  <button
                                    className="text-button"
                                    type="button"
                        ...
                        {selectedQuestion.risk === "protected" ? (
                          <details className="detail-disclosure">
                            <summary>
                              ...
                              <span className="section-count">{selectedQuestion.reviews.length}</span>
                            </summary>
                            {selectedQuestion.reviews.length === 0 ? (
                              <p className="muted-copy">
                                {selectedQuestion.requiredReviewerRoles.length > 0
                                  ? `No policy review has been recorded. Required human roles: ${selectedQuestion.requiredReviewerRoles.join(", ")}.`
                                  : "This protected policy is satisfied by its required owner role and has no separate reviewer role."}
                              </p>
                            ) : (
                              <div className="response-list">
                                {selectedQuestion.reviews.map((review) => (
                                  <article className="response-card" key={review.id}>
                                    ...
                                    <small>
                                      {new Date(review.createdAt).toLocaleString()} · {review.reviewerRole}
                                    </small>
                                    <div className="response-rationale">
                                      <strong>Rationale:</strong> {review.rationale}
                                    </div>
                                  </article>
                                ))}
                              </div>
                            ...
                            {selectedQuestion.approvalOverride ? (
                              <div className="accepted">
                                ...
                                {selectedQuestion.approvalOverride.changedById} ·{" "}
                                {new Date(selectedQuestion.approvalOverride.createdAt).toLocaleString()}
                            ...
                            {selectedQuestion.canOverrideApproval ? (
                              <div className="response-form">
                                ...
                                <p className="muted-copy">
                                  Use only when the protected requirement cannot be satisfied through the configured
                                  human reviewer route. This action is audited under your identity.
                                </p>
                        ...
                        {selectedQuestion.status === "accepted" ? (
                          <div className="accepted">
                            <strong>Decision accepted.</strong> Future agent context requests can retrieve{" "}
                            {selectedQuestion.decisionId}.
                          </div>
                        ) : !selectedQuestion.canAccept ? (
                          <div className="owner-routing">
                            <strong>Shared review only.</strong> Add a response here; the configured owner or required
                            security reviewer must accept the decision from My Inbox.
                          </div>
                        ) : (
                          <section className="approval-panel">
                            <div className="approval-heading">
                              <span>
                                <BridgeIcon name="decisions" size={20} />
                              </span>
                              <div>
                                <h3>Human approval</h3>
                                <p>
                                  Accepting creates the authoritative Bridge decision. The agent recommendation remains
                                  advisory.
                                </p>
                              </div>
                            </div>
            ...
            <>
              <div className="title-row">
                <div>
                  <span className="page-eyebrow">Governed documents</span>
                  <h1>Specification reviews</h1>
                  <p>Read agent-published drafts as clear documents, then review or approve one immutable version.</p>
                </div>
                <button
                  className="secondary"
                  type="button"
                  onClick={() => void Promise.all([loadArtifacts(), loadNotifications()])}
                >
                  Refresh
                </button>
              </div>
              ...
              {!artifactsLoading && artifacts.length === 0 ? (
                <div className="empty">No specifications have been published.</div>
              ) : null}
                        ...
                        <button
                          ...
                          className={
                            artifact.id === selectedArtifactId
                              ? "question-row specification-row selected"
                              : "question-row specification-row"
                          }
                          onClick={() => setSelectedArtifactId(artifact.id)}
                        >
                          <span className="document-mark" aria-hidden="true">
                            <BridgeIcon name="specifications" size={16} />
                          </span>
                          <span>
                            <strong>{artifact.title}</strong>
                            <small>
                              {artifact.type.replaceAll("_", " ")} · version {version?.version ?? "?"}
                            </small>
                          </span>
                          <span className={`status status-${displayedStatus}`}>
                    ...
                    <article className="question-detail specification-detail">
                      <header className="specification-header">
                        <div className="specification-title-block">
                          <span className="specification-type">
                            <BridgeIcon name="specifications" size={15} />
                            {selectedArtifact.type.replaceAll("_", " ")}
                          </span>
                          <h2>{selectedArtifact.title}</h2>
                          <small title={selectedArtifact.id}>
                            Immutable governed record · published{" "}
                            {new Date(selectedArtifactVersion.createdAt).toLocaleDateString()}
                          </small>
                        </div>
                        <div className="specification-state">
                          <span className={`status status-${displayedArtifactStatus(selectedArtifactVersion)}`}>
                            {displayedArtifactStatus(selectedArtifactVersion).replaceAll("_", " ")}
                          </span>
                      ...
                      <section className="specification-overview" aria-labelledby="review-brief-title">
                        <div className="spec-brief">
                          ...
                          <div className="spec-approval-meter">
                            ...
                            <strong>
                              {selectedArtifactVersion.approvalStatus.approvedCount}
                              <small> / {selectedArtifactVersion.approvalStatus.requiredCount}</small>
                            </strong>
                            <div aria-hidden="true">
                              <span
                                style={{
                                  width: `${Math.min(100, (selectedArtifactVersion.approvalStatus.approvedCount / Math.max(1, selectedArtifactVersion.approvalStatus.requiredCount)) * 100)}%`,
                                }}
                              />
                            </div>
                            <small>
                              {selectedArtifactVersion.approvalStatus.remainingCount === 0
                                ? "Quorum complete"
                                : `${selectedArtifactVersion.approvalStatus.remainingCount} remaining`}
                            </small>
                          </div>
                        </div>
                        <dl className="spec-meta-line">
                          <div>
                            <dt>Publisher</dt>
                            <dd title={selectedArtifactVersion.createdById}>
                              {displayIdentityName(selectedArtifactVersion.createdById, principals)}
                            </dd>
                          </div>
                          <div>
                            <dt>Human reviewers</dt>
                            <dd title={selectedArtifactReviewerIds.join(", ")}>
                              {selectedArtifactReviewerIds
                                .map((reviewerId) => displayIdentityName(reviewerId, principals))
                                .join(", ") || "Not assigned"}
                            </dd>
                          </div>
                          <div>
                            <dt>Scope</dt>
                            <dd>
                              {selectedArtifact.scope.component ?? selectedArtifact.scope.repository ?? "Project"}
                            </dd>
                          </div>
                          <div>
                            <dt>Version</dt>
                            <dd>{selectedArtifactVersion.version}</dd>
                          </div>
                        </dl>
                        <details className="spec-routing-disclosure">
                          <summary>
                            ...
                            <small>
                              {selectedArtifactVersion.reviewerAssignment?.routeSource.replaceAll("_", " ") ??
                                "Legacy record"}
                            </small>
                          </summary>
                          {selectedArtifactVersion.reviewerAssignment ? (
                            <dl>
                              <div>
                                <dt>Route</dt>
                                <dd className="routing-source">
                                  {selectedArtifactVersion.reviewerAssignment.routeSource.replaceAll("_", " ")}
                                </dd>
                              </div>
                              <div>
                                <dt>Ownership version</dt>
                                <dd>{selectedArtifactVersion.reviewerAssignment.ownershipVersion}</dd>
                              </div>
                              <div>
                                <dt>Route detail</dt>
                                <dd
                                  title={
                                    selectedArtifactVersion.reviewerAssignment.sourceAssignmentId ??
                                    selectedArtifactVersion.reviewerAssignment.ownershipRuleKey
                                  }
                                >
                                  {selectedArtifactVersion.reviewerAssignment.sourceAssignmentId ??
                                    selectedArtifactVersion.reviewerAssignment.ownershipRuleKey ??
                              ...
                              <div>
                                <dt>Assignment</dt>
                                <dd title={selectedArtifactVersion.reviewerAssignment.id}>
                                  {selectedArtifactVersion.reviewerAssignment.id}
                                </dd>
                              </div>
                            </dl>
                          ) : (
                            <p>
                              This version predates immutable reviewer routing. Bridge is showing the artifact's current
                              reviewer list as a compatibility fallback.
                            </p>
                          )}
                        </details>
                      </section>
                      ...
                      <section className="specification-reader" aria-labelledby="specification-document-title">
                        <div className="spec-reader-heading">
                          <span className="reader-mark">
                            <BridgeIcon name="specifications" size={18} />
                          </span>
                          <div>
                            <span>Readable document</span>
                            <h3 id="specification-document-title">Specification content</h3>
                          </div>
                        ...
                        <div className="spec-paper">
                          <MarkdownDocument
                            ...
                            omitLeadingHeading
                            source={selectedArtifactVersion.body}
                          />
                        </div>
                        <details className="spec-source-disclosure">
                          <summary>
                            View immutable Markdown source <span>Technical view</span>
                          </summary>
                      ...
                      <div className="specification-controls">
                        <details className="detail-disclosure">
                          <summary>
                            Review feedback{" "}
                            <span className="section-count">{selectedArtifactVersion.reviews.length}</span>
                          </summary>
                          ...
                          {canReviewSelectedArtifact &&
                          ["draft", "in_review"].includes(selectedArtifactVersion.status) ? (
                            <div className="response-form">
                              ...
                              <select
                                ...
                                onChange={(event) =>
                                  setArtifactReviewStatus(event.target.value as typeof artifactReviewStatus)
                                }
                              >
                              ...
                              <button
                                ...
                                disabled={
                                  submitting ||
                                  artifactReviewBody.trim().length <
                                    (artifactReviewStatus === "changes_requested" ? 10 : 2)
                                }
                                onClick={() => void reviewSpecification()}
                              >
                                {submitting
                                  ? "Recording feedback…"
                                  : artifactReviewStatus === "changes_requested"
                                    ? "Request changes"
                                    : "Post review comment"}
                              </button>
                            </div>
                          ...
                          {selectedArtifact.versions.length < 2 ? (
                            ...
                            <>
                              <div className="diff-controls">
                                ...
                                >
                                  {selectedArtifact.versions.map((version) => (
                                    <option key={version.id} value={version.id}>
                                      ...
                                      {displayedArtifactStatus(version).replaceAll("_", " ")}
                                    </option>
                                  ))}
                                </select>
                                ...
                                >
                                  {artifactDiffLoading ? "Comparing…" : "Compare"}
                                </button>
                              </div>
                              {artifactDiff ? (
                                <div className="artifact-diff">
                                  <div className="diff-summary">
                                    <strong>
                                      Version {artifactDiff.from.version} → {artifactDiff.to.version}
                                    </strong>
                                  ...
                                  <div className="diff-version-meta">
                                    <span>
                                      ...
                                      <small>
                                        {artifactDiff.from.createdById} ·{" "}
                                        {new Date(artifactDiff.from.createdAt).toLocaleString()}
                                      </small>
                                    </span>
                                    <span>
                                      ...
                                      <small>
                                        {artifactDiff.to.createdById} ·{" "}
                                        {new Date(artifactDiff.to.createdAt).toLocaleString()}
                                      </small>
                                    </span>
                                  </div>
                                  {!artifactDiff.exact ? (
                                    <div className="impact">
                                      <strong>Large comparison.</strong> The changed middle is shown as bounded removals
                                      and additions to protect server and browser performance.
                                    </div>
                                  ) : null}
                                  {artifactDiff.truncated ? (
                                    <div className="impact">
                                      <strong>Display limited.</strong> Showing {artifactDiff.lines.length} of{" "}
                                      {artifactDiff.totalLines} diff lines.
                                    </div>
                                  ) : null}
                                  <div
                                    className="diff-view"
                                    role="table"
                                    aria-label={`Specification diff from version ${artifactDiff.from.version} to ${artifactDiff.to.version}`}
                                  >
                                  ...
                                  <p className="muted-copy">
                                    Changed Markdown appears as adjacent removed and added lines. Stored version bodies
                                    remain immutable.
                                  </p>
                                </div>
                              ) : null}
                          ...
                          <div className="version-list">
                            {[...selectedArtifact.versions].reverse().map((version) => (
                              <div key={version.id}>
                                ...
                                <span className={`status status-${displayedArtifactStatus(version)}`}>
                                  {displayedArtifactStatus(version).replaceAll("_", " ")}
                                </span>
                        ...
                        {selectedArtifactVersion.status === "approved" ? (
                          ...
                          <div className="impact">
                            <strong>Changes requested.</strong> This immutable version cannot be approved. Publish a new
                            version that addresses the feedback.
                          </div>
                        ) : canReviewSelectedArtifact && activePrincipalApprovedSelectedArtifact ? (
                          <div className="impact">
                            <strong>Your approval is recorded.</strong>{" "}
                            {selectedArtifactVersion.approvalStatus.remainingCount} more distinct human approval
                            {selectedArtifactVersion.approvalStatus.remainingCount === 1 ? " is" : "s are"} required.
                          </div>
                        ) : canReviewSelectedArtifact ? (
                          <section className="approval-panel specification-approval-panel">
                            <div className="approval-heading">
                              ...
                              <div>
                                <h3>Human specification approval</h3>
                                <p>
                                  Your approval applies only to immutable version {selectedArtifactVersion.version}.
                                  Agent publication does not grant approval authority.
                                </p>
                              </div>
                            </div>
                            ...
                            >
                              {submitting
                                ? "Recording approval…"
                                : `Record approval for version ${selectedArtifactVersion.version}`}
                            </button>
                          </section>
                        ) : (
                          <div className="owner-routing">
                            <strong>Shared review only.</strong> A configured specification reviewer or project
                            administrator must approve this version.
                          </div>
                        )}
                      </div>
                    </article>
